...
wl_pktfwd_rx_mcast_handler(struct wl_info *wl, struct wlc_if* wlcif,  void *skb)
{
    ...
    if (bsscfg && ! bsscfg->ap_isolate && PKTISINTRABSS_FWD(skb) && (tmp = PKTDUP(wl->osh, skb))) {
#else
    ...
    if (!ETHER_ISMULTI(d3_addr))
    {
        ...
        if (d3lut_elem != D3LUT_ELEM_NULL) {
            ...
            if (rtnl_trylock()) {
                ...
                dst_br_dev = netdev_master_upper_dev_get(dst_dev);
                rtnl_unlock();
            }
    ...
    if (likely(d3lut_elem != D3LUT_ELEM_NULL))
    {
        ...
        if (net_device != NULL) {
            ...
            if (rtnl_trylock()) {
                ...
                dst_br_dev = netdev_master_upper_dev_get(d3lut_elem->ext.virt_net_device);
                rtnl_unlock();
            }
