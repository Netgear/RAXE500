	...
	int max_bss)
{
	...
	if (dma_buf->va == NULL) {
	    ...
	    RDPA_INIT_CFG_SET_PA(dhd_init_cfg->tx_post_mgmt_arr_base_phys_addr, dma_buf->pa, 0);
	...
	if (dhd_hlp->rnr_en_feat.txoffl == 1) {
	    ...
	    if (dhd_hlp->rnr_en_feat.npm == 0) {
	        if (flowmgr->hw_mem_size < full_dor_sz) {
	            RLOG(CLRerr "========================================================");
	            RLOG("dor%d TxOffl Reserved memory < Full Offload memory, aborting",
	                dhd_hlp->dhd->unit);
	            RLOG("========================================================" CLRnl);
	            goto error_rtn;
	        }
	    ...
	    case H2R_MSGRING_FORMAT_NOTIF:
	            ...
	            (uint8)arg1, (uint32)arg2));
	        {
	            ...
	            if (!rx_dor_en) {
	               ...
	               if (DHD_RNR_RX_OFFLOAD(dhd_hlp)) {
	                    DHD_ERROR(("Force disabling Runner Offload for RX\n"));
