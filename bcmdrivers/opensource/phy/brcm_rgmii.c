...
static void bcm_misc_hw_xmii_pads_init(rgmii_params *params)
{
    ...
    for (num = 0; num < params->num_pins ;num++)
    {
        ...
        bcm_set_padctrl(params, params->pins[num], tp_data);
    }
}

int rgmii_attach(uint32_t port, rgmii_params *params)
{
    uint32_t val;
    ...
    switch (rgmii_ver) {
    case RGMII_VER1:    val = (params->delay_rx) ? 0x08 : 0x28; break;
    case RGMII_VER7:
