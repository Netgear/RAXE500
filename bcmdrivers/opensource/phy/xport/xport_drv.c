...
static int xport_msbus_init(xport_xlmac_port_info_s *init_params)
{
    ...
    tx_wrr_ctrl.p1_weight = 0;
    tx_wrr_ctrl.p2_weight = (xlmac_num)? 0 : 1; /* potentially 5G for 1st instance */
...
static int xport_platform_init(xport_xlmac_port_info_s *init_params)
{
    ...
    if (xport_num == 0)
    {
        ...
        rc = rc ? rc : ag_drv_xport_xlmac_reg_port_0_rxerr_mask_get(xlmac_num, &rsv_err_mask);
        rsv_err_mask &= 0xffffffdf;
        rc = rc ? rc : ag_drv_xport_xlmac_reg_port_0_rxerr_mask_set(xlmac_num, rsv_err_mask);
        rc = rc ? rc : ag_drv_xport_xlmac_reg_port_1_rxerr_mask_set(xlmac_num, rsv_err_mask);
        rc = rc ? rc : ag_drv_xport_xlmac_reg_port_2_rxerr_mask_set(xlmac_num, rsv_err_mask);
        rc = rc ? rc : ag_drv_xport_xlmac_reg_port_3_rxerr_mask_set(xlmac_num, rsv_err_mask);
    }
    // do per channel init
