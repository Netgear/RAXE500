...
{
    int LANE = LaneNum;
    print_log("MerlinSupport::merlin28_powerdown_lane(): powering down core #0x%x lane #0x%x\n", CoreNum, LaneNum);
...
{
    int LANE = LaneNum;
    print_log("MerlinSupport::merlin28_powerdown_lane(): powering up core #0x%x lane #0x%x\n", CoreNum, LaneNum);
...
static void save_regs(prog_seq_tbl *ent, int CoreNum, int LaneNum)
{
    static int entp[3] = {0};
    ...
    if (ent == NULL)
    {
        lane_active = (merlin28_pmi_read16(CoreNum, LaneNum, 1, 0xd081) & 1) > 0;
        if (lane_active)
            merlin28_pmi_write16(CoreNum, LaneNum, 1, 0xd081, 0, 0xfffe); /* Put Serdes in Reset first */
        for (i = 0; i < entp[CoreNum]; i++)
        {
            save_reg = &saved_regs[CoreNum][i];
            tbl_ent = save_reg->ent;
            merlin28_pmi_write16(CoreNum, LaneNum, tbl_ent->dev_addr, tbl_ent->reg_addr, save_reg->val, 0);
        }
        entp[CoreNum] = 0;
        if (lane_active)
            merlin28_pmi_write16(CoreNum, LaneNum, 1, 0xd081, 1, 0xfffe); /* Restore Lane Active Status */
        return;
    }
...
#define PRINT_DUPLICATED_REG_SAVE 0
    ...
    save_reg = &saved_regs[CoreNum][i];
    save_reg->ent = ent;
    save_reg->val = merlin28_pmi_read16(CoreNum, LaneNum, ent->dev_addr, ent->reg_addr);
}
#define restore_regs(CoreNum, LaneNum) save_regs(0, CoreNum, LaneNum)
    ...
    while(prog_seq_tbl_ptr->reg_desc[0])
    {
        ...
        if (strcmp(prog_seq_tbl_ptr->reg_desc, "timeout_100ns") == 0) {
            ...
            if(parse_sim_opts("-d USE_MDIO")) {
                ...
                save_regs(prog_seq_tbl_ptr, CoreNum, LANE);
...
static void merlin28_reset_datapath_core (uint32 CoreNum)
{
    int LANE = 0;
    print_log("%s(): Datapath Reset (core) in progress \n", __func__);
    merlin28_reg_prog(datapath_reset_core, CoreNum, 0x0);
}
...
static void merlin28_lane_config_speed(uint32 CoreNum, uint32 LaneNum, uint32 LNK_SPD, int an_enabled)
{
    int LANE = LaneNum;
...
static void merlin28_check_pll_lock (uint32 CoreNum, int LANE, bool VCO_9P375G)
{
    ...
    */
    ...
    while (!pll_lock) {
        #if 0
        if (parse_sim_opts("-d MERLIN_BPCM")) {
            bpcm_rd_addr =  SERDES_0_BPCM_STATUS;
            bpcm_rd_data = switch_blk->bpcm_regs->read32(bpcm_rd_addr);
            //print_log("INFO merlin28_core_init(): BPCM: READ %x from %x\n", bpcm_rd_data, bpcm_rd_addr);
            pll_lock = bpcm_rd_data & (0x1 << MERLIN_BPCM_SERDES_STATUS_PLL_LOCK_OFFSET);   //[6]: PLL_LOCK
        } else
        #endif
        {
        ...
        //print_log("%s(): DEBUG INFO: Core #%d read %x from %x, and PLL Lock Status = %d \n", __func__, CoreNum, rd_data, rd_addr, pll_lock);
        msleep(1);
        watch_dog = watch_dog + 1;
        if (watch_dog > watch_dog_timer) {
            fatal_log("ERROR !!! %s(): Merlin Core #%d PLL lock checking has timed-out (rd_data=0x%x, pll_lock=0x%x) \n", __func__, CoreNum, rd_data, pll_lock);
        }
    }
...
static int merlin28_speed_set_core(phy_dev_t *phy_dev)
{
    ...
    restore_regs(CoreNum, phy_dev->lane_index);  // Restore all registers to default values for new speed programing

    vco_9p375g = phy_mode == INTER_PHY_TYPE_2500BASE_X;

    //---Step 5. PLL Configuration
    print_log("%s(): Step 8. PLL Configuration \n", __func__);
    if (vco_9p375g) {
        print_log("%s(): Initialize 9.375G VCO programming \n", __func__);
        merlin28_reg_prog(Initialize_9p375_VCO, CoreNum, 0x0);
    }
    //--- PLL/PMD setup configuration
    if (parse_sim_opts("-d ML_REFCLK_156P25")) {
        if (vco_9p375g) {
            print_log("%s(): PMD Setup 156.25MHz, 12.5GHz VCO programming \n", __func__);
            merlin28_reg_prog(PMD_setup_156p25_9p375_VCO, CoreNum, 0x0);
        } else {
            print_log("%s(): PMD Setup 156.25MHz, 10.3125GHz VCO programming \n", __func__);
            merlin28_reg_prog(PMD_setup_156p25_10p3125_VCO, CoreNum, 0x0); //CoreNum = 0x1f for broadcast
        }
    ...
    merlin28_reg_prog(PMD_set_bypass_tx_osr_lane, CoreNum, phy_dev->lane_index);
    ...
    merlin28_reg_prog(en_datapath_reset_lane, CoreNum, phy_dev->lane_index);
    ...
    merlin28_reset_datapath_core (CoreNum);

    //Program lane's speed mode
    merlin28_lane_config_speed(CoreNum, phy_dev->lane_index, phy_serdes->serdes_speed_mode, phy_dev->an_enabled);

    //--- --- check PLL lock status
    merlin28_check_pll_lock (CoreNum, phy_dev->lane_index, vco_9p375g); // pass lane just for logging 
    ...
    /* Special dealy needed for certain speed */
    /*
    if (phy_dev->current_inter_phy_type == INTER_PHY_TYPE_2500BASE_X ||
...
int merlin28_speed_set(phy_dev_t *phy_dev, phy_speed_t speed, phy_duplex_t duplex)
{
    ...
    return merlin28_speed_set_core(phy_dev);
}
...
int merlin28_serdes_init(phy_dev_t *phy_dev)
{
    ...
    /* Power Down all lanes and power up current lane */
    for (lane = 0; lane < MAX_LANE_PER_CORE; lane++)
