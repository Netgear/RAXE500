...
// TODO: remove after we have completely switched over to 4.19.  Using const
// is the correct way, but Linux 4.1 has an internal inconsistency which
// prevents it.
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,0))
#define COND419_CONST const
#else
#define COND419_CONST 
#endif

static int ubus4_dcm_enable_get (char *buffer, COND419_CONST struct kernel_param *kp)
{
    return sprintf(buffer, "%d\t : valid value { 0, 1}", dcm_enable_get());
}

static int ubus4_dcm_enable_set(const char *val, COND419_CONST struct kernel_param *kp)
{
    if (!strncmp(val, "0", 1)) {
...
static int ubus4_dcm_divider_get (char *buffer, COND419_CONST struct kernel_param *kp)
{
...
static int ubus4_dcm_divider_set(const char *val, COND419_CONST struct kernel_param *kp)
{
	u32 ival;
...
static int ubus4_dcm_thold_get (char *buffer, COND419_CONST struct kernel_param *kp)
{
    return sprintf(buffer, 
		"0x%04x\t : valid value {1..0xFFFF}",dcm_thold_get());
}

static int ubus4_dcm_thold_set(const char *val, COND419_CONST struct kernel_param *kp)
{
	u32	ival;
...
#ifdef DEBUG
static int ubus4_dcm_config_get (char *buffer, COND419_CONST struct kernel_param *kp)
{
...
static int ubus4_dcm_config_set(const char *val, COND419_CONST struct kernel_param *kp)
{
	dcm_config_set();
    return 0;
}
