...
#if !defined(CONFIG_BCM963158) && !defined(CONFIG_BCM_DSL_RDP)/* All PON Platforms + 63146, 4912 */
/* scratchpad defaults */
...
#if !defined(CONFIG_BCM963158) && !defined(CONFIG_BCM_DSL_RDP) && !defined(CONFIG_BRCM_QEMU)/* All PON Platforms + 63146, 4912 */
...
static int rdpa_get_init_system_bp_params(rdpa_emac *gbe_wan_emac)
{
    ...
    if (EnetInfos == NULL)
        return rc;
    emac_map = EnetInfos[0].sw.port_map & 0xFF;
    gbewan_scratchpad_get(gbe_wan_emac);
#endif
#else
    emac_map = 0xF;
#endif
...
#if !defined(CONFIG_BCM963158) && !defined(CONFIG_BRCM_QEMU)/* All PON Platforms + 63146, 4912*/
static int gbewan_scratchpad_get(rdpa_emac *wan_emac)
{
    int rc;
...
int operation_mode_get(rdpa_operation_mode *operation_mode)
{
    ...
    rc = scratchpad_get_or_init(RDPA_OPERATION_MODE_PSP_KEY, buf, sizeof(buf), NULL, scratchpad_func_get);
    if (rc < 0)
        return rc;
    if (!(strlen(buf) > 0))
    {
        ...
        if (rc < 0)
            return rc;
        if ((!strcasecmp(buf ,"FC")) || (!strcasecmp(buf ,"MIXED")) || (!strcasecmp(buf ,"NONE")))
        {
            if (!strcasecmp(buf ,"NONE"))
                rc = scratchpad_get_or_init(RDPA_OPERATION_MODE_PSP_KEY, NULL, 0, "PRV", scratchpad_func_init);
            else
                rc = scratchpad_get_or_init(RDPA_OPERATION_MODE_PSP_KEY, NULL, 0, "FC", scratchpad_func_init);
            if (rc < 0)
                return rc;
        }
    }
...
{
#if defined(CONFIG_BCM96878) || defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912)
    int rc;
    uint32_t val;
    char buf[PSP_BUFLEN_16];
#endif
