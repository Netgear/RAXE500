...
void __init ses_board_init()
{
    ...
    if (register_button_action("ses_button", "ses_short_period", sesBtn_releaseAction) < 0)
    {
        printk("ses_board_init: cannot register ses_short_period action for ses_button\n");
        return;
    }

    if (register_button_action("ses_button", "ses_long_period", sesBtn_releaseAction) < 0)
    {
...
#else
           int rl=kernel_read(fp,0,(size_t)wlanParams, (loff_t *)&len);
#endif
           filp_close(fp,NULL);
           if(rl<=0) 
               return -1;
