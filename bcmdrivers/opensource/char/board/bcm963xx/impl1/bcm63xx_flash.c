...
{
    char env_location[128]; // 128 bytes is enough to hold more many backup locations  
    char *ptr=NULL;
    unsigned int address;
    int ret=0;

    //get env_boot_magic=16384@0x40000,0xa0000
    if (eNvramGet("env_boot_magic", env_location, sizeof(env_location)) > 0)
    {
        ptr=strchr(env_location, '@');
        if(ptr)
        {
            ptr++;
            while(ptr)
                ...
                if(address)
                {
                    switch(flash_type)
                    {
                        ...
                        case FLASH_IFC_SPINAND:
                            ...
                            break;
                        case FLASH_IFC_UNSUP_EMMC:
                            //FIXME: Assuming boot0 partition, must figure out current boot partition index in the future
                            ret = kerSysFsFileSetOffset("/sys/block/mmcblk0boot0/force_ro", "0", strlen("0"), 0 );
                            ret = kerSysFsFileSetOffset("/dev/mmcblk0boot0", (char*)input_data, length, (loff_t)address );
                            break;
                    }
                    if(ret != 0)
                    {
