                                           ...
                                           uintptr_t          flowhdl)
{
    ...
    if ( bcm_mcast_flow_add_hook(new_blog_p, traffic, &flow_key) )
    {
        ...
        if (BCM_MCAST_PROTO_IPV4 == proto) 
        {
            __logInfo("blog_activate() failed Grp 0x%x vlan0 0x%x vlan1 0x%x numtags %d rxdev %s txdev %s",
        ...
        {
            __logInfo("blog_activate() failed vlan0 0x%x vlan1 0x%x numtags %d rxdev %s txdev %s",
                       new_blog_p->vtag[0], new_blog_p->vtag[1], new_blog_p->vtag_num, 
                      ((struct net_device *)rxDev)->name, ((struct net_device *)txDev)->name);
    ...
    else
    {
        if (BCM_MCAST_PROTO_IPV4 == proto) 
        {
            __logInfo("blog_activate() successful Group 0x%x vlan0 0x%x vlan1 0x%x numtags %d rxdev %s txdev %s",
                       htonl(new_blog_p->rx.tuple.daddr), new_blog_p->vtag[0], new_blog_p->vtag[1], 
                       new_blog_p->vtag_num, ((struct net_device *)rxDev)->name, ((struct net_device *)txDev)->name);
        }
        else
        {
            __logInfo("blog_activate() successful vlan0 0x%x vlan1 0x%x numtags %d rxdev %s txdev %s",
                       new_blog_p->vtag[0], new_blog_p->vtag[1], new_blog_p->vtag_num, 
                      ((struct net_device *)rxDev)->name, ((struct net_device *)txDev)->name);
