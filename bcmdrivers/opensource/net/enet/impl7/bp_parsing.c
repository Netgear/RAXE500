...
static int __init bp_parse_port(enetx_port_t *sw, const ETHERNET_MAC_INFO *emac_info, uint32_t port, uint32_t unit, uint32_t *oam_idx_cnt)
{
...

    {
        ...
        if (p->p.phy)
        {
            switch (p->p.phy->phy_drv->phy_type)
            {
                case PHY_TYPE_138CLASS_SERDES:
                case PHY_TYPE_158CLASS_SERDES:
                case PHY_TYPE_146CLASS_SERDES:
                    p->p.phy->core_index = chip_arch_serdes_core_num[port];
                    break;
                default:
                    p->p.phy->core_index = 0;
            }

            if (p->p.phy->core_index == -1)
            {
                enet_err("****** Wrong Serdes Port defined at %d in board paramter.\n", p->p.phy->addr);
                BUG();
            }
            p->p.phy->sw_port = p;
            enet_dbgv("bp_parse_phy_dev: phyId=%x (%s) %px bus_drv:%px\n", p->p.phy->addr, p->p.phy->phy_drv->name, p->p.phy, p->p.phy->bus_drv);
        }
...
static int __init bp_parse_qemu(void)
{
  ...
  {
    enetx_port_t *p_2;
    port_info_t port_info_2 ={
        .port = 2,
        .is_wan = 0,
    };

    if (port_create(&port_info_2, sw, &p_2) != 0)
       printk("Port Create Error:[%s:%d]\n",__FUNCTION__,__LINE__);

    p_2->has_interface = 1;
    p_2->p.mac = mac_dev_add(MAC_TYPE_UNIMAC, 2, (void*)(1<<0)/*UNIMAC_DRV_PRIV_FLAG_GMII_DIRECT*/);
    if (p_2->p.mac == NULL)
       printk("Add MAC to dev Error:[%s:%d]\n",__FUNCTION__,__LINE__); 

    unit_port_array[0][0] = p_0;
    unit_port_array[0][1] = p_1;
    unit_port_array[0][2] = p_2;
  }
#endif
    return 0;
}
