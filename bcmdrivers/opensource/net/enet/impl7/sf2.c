...
int ioctl_extsw_port_erc_config(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_SET) {
        ...
        /* Enable/disable IFG Correction Control */
        SF2SW_RREG(e->unit, page, SF2_REG_SHAPER_IFG_BYTES, &val16, 2);
        val16 &= ~(1 << e->port); /* Disable Shaper */
        val16 |= e->limit? (1 << e->port): 0; /* Enable IFG BYTE correction, if needed */
        SF2SW_WREG(e->unit, page, SF2_REG_SHAPER_IFG_BYTES, &val16, 2);
        ...
        if (!q_shaper) {
            SF2SW_RREG(e->unit, page, SF2_REG_SHAPER_INC_IFG_CTRL, &val16, 2);
...
int ioctl_extsw_port_shaper_config(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_SET) {
        ...
        if (e->sub_type & SHAPER_ENABLE) {
            ...
            _extsw_reg16_bit_ops(e->unit, page, reg, e->port, e->val);
        }
...
static int tr_find_sf2_sw(enetx_port_t *sw, void *_ctx)
{
    ...
    if (sw->s.parent_port && sw->s.parent_port->p.phy && sw->s.parent_port->p.phy->reset_gpiod)
    {
        struct gpio_desc *gpio = sw->s.parent_port->p.phy->reset_gpiod;
...
void port_sf2_dual_tx_shape(enetx_port_t *port, int speed)
{
    if (port->p.parent_sw == sf2_sw_ext)
    {
        ...
        // setup WRR for all Qs
        e.port_qos_sched.sched_mode = BCM_COSQ_WRR;
        e.port_qos_sched.wrr_type = QOS_ENUM_WRR_PKT;
        e.port_qos_sched.num_spq = 0;
        e.port_qos_sched.weights_upper = 0;
        e.weights[0] = e.weights[1] = e.weights[2] = e.weights[3] = 1;
        ioctl_extsw_cosq_sched(unit_port_array[e.unit][e.port], &e);
        enet_dbgv("set ext_sw parent port queues to WRR\n");
    }
}
#endif //SF2_DUAL
