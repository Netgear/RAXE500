...
static int tr_port_by_net_port(enetx_port_t *port, void *_ctx)
{
    ...
    switch (net_port)
    {
        ...
        case NET_PORT_LAN_0 ... NET_PORT_LAN_7:
            if (!port->port_info.is_gpon && !port->port_info.is_epon && port->p.mac && port->p.mac->mac_id == net_port)
            {
                *(enetx_port_t **)_ctx = port;
                return 1;
            }
...
static char *_print_phy_info(phy_dev_t *phy_dev, char *bf, int indent)
{
    ...
    n += snprintf(bf+n, sz-n, "PHY_ID:0x%02x:%6s %5s %5s %8s %12s %s %s",
            phy_dev->addr,
            (IsExtPhyId(phy_dev->meta_id)||phy_dev->phy_drv->phy_type==PHY_TYPE_I2C_PHY)? "Extrnl":"OnChip",
