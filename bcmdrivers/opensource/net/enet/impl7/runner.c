...
int _runner_rdpa_dos_ctrl(struct ethswctl_data *e)
{
    ...
    if ((rc = rdpa_system_dos_attack_reason_get(system_obj, &dos_attck_reason)))
    {
    ...
    if (e->type == TYPE_GET)
    {
        if (dos_attck_reason & (1 << rdpa_dos_reason_mac_sa_eq_da))  e->dosCtrl.da_eq_sa_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_ip_land))  e->dosCtrl.ip_lan_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_tcp_blat))  e->dosCtrl.tcp_blat_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_udp_blat))  e->dosCtrl.udp_blat_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_tcp_null_scan))  e->dosCtrl.tcp_null_scan_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_tcp_xmas_scan))  e->dosCtrl.tcp_xmas_scan_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_tcp_synfin_scan))  e->dosCtrl.tcp_synfin_scan_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_tcp_syn_error))  e->dosCtrl.tcp_synerr_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_tcp_short_hdr))  e->dosCtrl.tcp_shorthdr_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_tcp_frag_error))  e->dosCtrl.tcp_fragerr_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_icmpv4_fragment))  e->dosCtrl.icmpv4_frag_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_icmpv6_fragment))  e->dosCtrl.icmpv6_frag_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_icmpv4_long_ping))  e->dosCtrl.icmpv4_longping_drop_en = 1;
        if (dos_attck_reason & (1 << rdpa_dos_reason_icmpv6_long_ping))  e->dosCtrl.icmpv6_longping_drop_en = 1;
    }
    else if (e->type == TYPE_SET)
    {
        dos_attck_reason = 0;
        if (e->dosCtrl.da_eq_sa_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_mac_sa_eq_da);
        if (e->dosCtrl.ip_lan_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_ip_land);
        if (e->dosCtrl.tcp_blat_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_tcp_blat);
        if (e->dosCtrl.udp_blat_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_udp_blat);
        if (e->dosCtrl.tcp_null_scan_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_tcp_null_scan);
        if (e->dosCtrl.tcp_xmas_scan_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_tcp_xmas_scan);
        if (e->dosCtrl.tcp_synfin_scan_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_tcp_synfin_scan);
        if (e->dosCtrl.tcp_synerr_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_tcp_syn_error);
        if (e->dosCtrl.tcp_shorthdr_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_tcp_short_hdr);
        if (e->dosCtrl.tcp_fragerr_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_tcp_frag_error);
        if (e->dosCtrl.icmpv4_frag_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_icmpv4_fragment);
        if (e->dosCtrl.icmpv6_frag_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_icmpv6_fragment);
        if (e->dosCtrl.icmpv4_longping_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_icmpv4_long_ping);
        if (e->dosCtrl.icmpv6_longping_drop_en) dos_attck_reason |= (1 << rdpa_dos_reason_icmpv6_long_ping);

        printk("SET dos_val 0x%04x \n", (uint16_t)dos_attck_reason);
        rc = rdpa_system_dos_attack_reason_set(system_obj, dos_attck_reason);
        if (rc)
        {   
            enet_err("Failed to set dos attack reason, v=0x%04x, rc=%d\n", (uint16_t)dos_attck_reason, rc);
            goto exit;
        }
    }
...
#endif
    net_stats->tx_packets = wan_port_stat.tx_unicast_pkt + wan_port_stat.tx_broadcast_pkt
        + wan_port_stat.tx_multicast_pkt;
