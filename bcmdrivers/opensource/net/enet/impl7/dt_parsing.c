...
static int of_port_probe(struct device *dev, enetx_port_t *sw, struct device_node *port_dn, int unit)
{
    ...
    if (!port_reg)
    {
        enet_err("Missing reg proerty\n");
        return -1;
    }

    port_index = be32_to_cpup(port_reg);
    port_info.port = port_index;
    port_info.is_epon = is_detect;
...
#ifndef ENET_DT_TEST
        phy_dev_attach(p->p.phy, mii_type, !params.delay_rx, !params.delay_tx);
#endif
#if defined(DSL_DEVICES)
    ...
    if ((mac_type_str = of_get_property(port_dn, "mac-type", NULL)))
    {
        ...
        if (mii_type == PHY_MII_TYPE_RGMII)
        {
            ...
            mac_dev_rgmii_attach(p->p.mac, &params);
        }
    }
    ...
    bca_led_request_network_leds(port_dn, &p->p.leds_info);
#endif
...
int of_sw_probe(struct device *dev, struct device_node *np, enetx_port_t *parent_port)
{
    ...
    sw->s.strap_msg = (char *)of_get_property(np, "strap-msg", NULL);
    return 0;
}
