...
static int runner_xtm_objects_init(void)
{
   ...
   if (rc)
   {
      BCM_LOG_NOTICE(BCM_LOG_ID_XTM, "Failed to init extended objects for WAN %s, error %d", bdmf_object_name(wan), rc);
      return rc;
   }
...
UINT32 bcmxapi_rxtask(UINT32 ulBudget, UINT32 *pulMoreToDo)
{
   ...
   */
   do
   {
      ...
      BCM_XTM_RX_DEBUG("Rcvd packet ulVcId:%u ulCell:%u pDevCtx:%pk",ulVcId,ulCell,pDevCtx);
      ...
      else if (!ulCell) /* process packet, pDev != NULL */
      {
         ...
         bcmxtmrt_processRxPkt(pDevCtx, NULL, (UINT8 *)info.data,
                               ...
                               info.reason == rdpa_cpu_rx_reason_mcast_exception? 1 : 0);
         ulRxPktGood++;
         ulBudget--;
      }
      else  /* process cell */
