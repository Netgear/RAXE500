...
{
   proc_create("svc-queues", 0440, dir, &svc_queues_fops);
   return 0;
}
...
{
   remove_proc_entry("svc-queues", dir);
   return 0;
...
static int create_service_queues(bdmf_object_handle tm)
{
   ...
   if (sub_sched)
   {
      BCM_LOG_NOTICE(BCM_LOG_ID_XTM, "sub_sched is set");
   ...
   {
      BCM_LOG_ERROR(BCM_LOG_ID_XTM, "Failed to create service queues scheduler, rc %d", rc);
      return rc;
   }
   ...
   for (i = 0; i < ARRAY_SIZE(svc_queues); i++)
   {
      ...
      if (rc || !svc_queues[i])
      {
         BCM_LOG_ERROR(BCM_LOG_ID_XTM, "Failed to create service queue %d, rc %d", i, rc);
         continue;
      }
