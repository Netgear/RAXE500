...
static void pmc_adjust_thermctl(void)
{
#if defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912)
#define BAC_CPU_THERMCTL_VAL	4
#define BAC_CPU_THERMCTL_MAS	0x3f
#define BAC_CPU_THERMCTL_SHT	22
	uint32_t reg;

	reg = readl(g_pmc->bac_cpu_base + BAC_CPU_THERMCTL_OFFSET);
	reg &= ~(BAC_CPU_THERMCTL_MAS << BAC_CPU_THERMCTL_SHT);
	writel(reg, g_pmc->bac_cpu_base + BAC_CPU_THERMCTL_OFFSET);
	
	reg |= (BAC_CPU_THERMCTL_VAL << BAC_CPU_THERMCTL_SHT);
	writel(reg, g_pmc->bac_cpu_base + BAC_CPU_THERMCTL_OFFSET);
...
#if defined(CONFIG_BCM96878) && !defined(CONFIG_BRCM_IKOS)
	pmc_tracking_init();
#endif
