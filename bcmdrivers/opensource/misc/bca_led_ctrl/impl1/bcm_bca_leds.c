...
static void common_bca_led_set(struct led_classdev *led_cdev, enum led_brightness value, int is_cled)
{
    ...
    case LED_OFF:
        ...
        break;
    case LED_FULL:
        led_set_brightness(led->pin, full_bright);
    case LED_ON:
        led_set_value(led->pin, 1);
        break;
    case LED_HALF:
        led_set_brightness(led->pin, half_bright);
        led_set_value(led->pin, 1);
    default:
        break;
    }
...
void bca_led_request_network_leds(struct device_node *dn, bca_leds_info_t *led_info)
{
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        {
            led_err("HW LED assigned to port %d referenced from port %d node %s\n", port, led_info->port_id,
                led_node->name);
            continue;
        }
