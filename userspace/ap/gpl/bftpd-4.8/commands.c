	...
	*pp++ = 0;
	for (i = 0; commands[i].name; i++) {	/* Parse command */
		if (!strncasecmp(str, commands[i].name, strlen(commands[i].name))) {
			...
			/* Foxconn, Laider, 2017/03/20 */
			// For Netgear RP-220: PASS command, only remove the space of format to keep the
			// head space of the password.
			if (!strncmp(commands[i].name, "PASS", sizeof(commands[i].name))) {
				cutto(str, strlen(commands[i].name) + 1); //skip the space of format
				p = str;
				while ((*p) && (*p == '\t'))
					p++;
			} else {
