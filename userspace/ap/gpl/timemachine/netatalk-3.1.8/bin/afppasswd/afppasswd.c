...
#define OPT_ADDPASS (1 << 5)

#define PASSWD_ILLEGAL '*'

#define FORMAT  ":****************:****************:********\n"
#define FORMAT_LEN 44
#define OPTIONS "cafnu:p:P:"
...
static int update_passwd(const char *path, const char *name, int flags, char* custom_password)
{
...
found_entry:
  ...
  if ((flags & OPT_ADDPASS)) {
     if(strlen(custom_password)<=0) {
        err = -1;
        goto update_done;
     }
     ...
     convert_passwd(p, custom_password, keyfd);
     lock.l_type = F_WRLCK;
     lock.l_start = pos;
     lock.l_len = 1;
...
int main(int argc, char **argv)
{
  ...
  if (flags & OPT_CREATE) {
    ...
    if (pwd)
      return update_passwd(path, pwd->pw_name, flags, custom_password);
