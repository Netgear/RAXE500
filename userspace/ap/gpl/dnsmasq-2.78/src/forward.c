...
#if (defined OPENDNS_PARENTAL_CONTROL)
#ifdef CONFIG_FUNJSQ
static int get_mac_from_dns_option(char *src_mac, int len, unsigned char *pheader, struct dns_header *header, size_t plen)
{
    unsigned char* ansp, save, tmp_ansp;
    unsigned short fun_op, fun_op_len, class, rdlen, type;
    ...
    GETSHORT(fun_op, ansp);
    if(fun_op==65001)
    {
        GETSHORT(fun_op_len, ansp);
        if(fun_op_len==6)
        {
            snprintf(src_mac, len-1, "%02x%02x%02x%02x%02x%02x", \
                ansp[0], ansp[1], ansp[2], ansp[3], ansp[4], ansp[5]);
        }
    }
...
#ifdef CONFIG_FUNJSQ
      if(strlen(dnsquery_src_mac)==0) {
          size_t pheader_len;
          unsigned char *pheader;
          pheader = find_pseudoheader(header, plen, &pheader_len, NULL, NULL ,NULL);
          get_mac_from_dns_option(dnsquery_src_mac, sizeof(dnsquery_src_mac), pheader, header, plen);
      }
#endif
