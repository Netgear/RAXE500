/* Shared library add-on to iptables to add BDCONNMARK target support.
 *
 *
 * Based on CONNMARK target:
 *
...
static void connmark_tg_check(struct xt_fcheck_call *cb)
{
	if (!(cb->xflags & F_OP_ANY))
		xtables_error(PARAMETER_PROBLEM,
		           "BDCONNMARK target: No operation specified");
}
                           ...
                           const struct xt_entry_target *target, int numeric)
{
	...
	case XT_CONNMARK_SET:
	    printf(" BDCONNMARK set ");
	    print_mark(markinfo->mark);
	    print_mask("/", markinfo->mask);
	    break;
	case XT_CONNMARK_SAVE:
	    printf(" BDCONNMARK save ");
	    print_mask("mask ", markinfo->mask);
	    break;
	case XT_CONNMARK_RESTORE:
	    printf(" BDCONNMARK restore ");
	    print_mask("mask ", markinfo->mask);
	    break;
	default:
	    printf(" ERROR: UNKNOWN BDCONNMARK MODE");
	    break;
	}
}
                  ...
                  int numeric)
{
	...
	case XT_CONNMARK_SET:
		if (info->ctmark == 0)
			printf(" BDCONNMARK and 0x%x",
			       (unsigned int)(uint32_t)~info->ctmask);
		else if (info->ctmark == info->ctmask)
			printf(" BDCONNMARK or 0x%x", info->ctmark);
		else if (info->ctmask == 0)
			printf(" BDCONNMARK xor 0x%x", info->ctmark);
		else if (info->ctmask == 0xFFFFFFFFU)
			printf(" BDCONNMARK set 0x%x", info->ctmark);
		else
			printf(" BDCONNMARK xset 0x%x/0x%x",
	...
	case XT_CONNMARK_SAVE:
		if (info->nfmask == UINT32_MAX && info->ctmask == UINT32_MAX)
			...
			printf(" BDCONNMARK save mask 0x%x", info->nfmask);
		else
			printf(" BDCONNMARK save nfmask 0x%x ctmask ~0x%x",
	...
	case XT_CONNMARK_RESTORE:
		if (info->ctmask == UINT32_MAX && info->nfmask == UINT32_MAX)
			...
			printf(" BDCONNMARK restore mask 0x%x", info->ctmask);
		else
			printf(" BDCONNMARK restore ctmask 0x%x nfmask ~0x%x",
