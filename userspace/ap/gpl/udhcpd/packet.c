...
static int input_vail_getstrtosys(char* int_str, char* out_str, int out_str_len_max);
...
static void run_daemon_dhcp_request_script(struct dhcpMessage *packet){
    ...
    char vail_optdata[1024] = {0};
    input_vail_getstrtosys(optdata, vail_optdata, sizeof(vail_optdata)-1);
      
    snprintf( cmd, sizeof(cmd), "%s %s '%s' '%s'",
                  ND_SCRIPT, "DHCP-REQUEST", mac, vail_optdata );
    system( cmd );
    return;
}
...
static int input_vail_getstrtosys(char* int_str, char* out_str, int out_str_len_max)
{
    /*Max out_str_len_max is strlen(int_str)*4 */
    int int_str_len = strlen(int_str);
    int i = 0;
    int j = 0;
    int ret = 0;

    for(i = 0; (i < int_str_len) && (j < out_str_len_max); i++, j++)
    {
        if(int_str[i] == '\'')
        {
            if(j+3 < out_str_len_max)
            {
                out_str[j++] = '\'';
                out_str[j++] = '\\';
                out_str[j++] = '\'';
    ...
    {
        ret = 1;
        printf("%s(%d) buffer is not enough!\n", __FUNCTION__, __LINE__);

        memset(out_str,'\0',out_str_len_max);
    }
    return ret;
}
