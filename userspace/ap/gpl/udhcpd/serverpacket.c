...
static int get_guest_assoclist(void)
{
	...
	if (info.uptime - last_update >= MAX_REFRESH_TIME) {
		...
		system(command);
#if defined(BRCM_EZ_MESH) /*foxconn Han edited 11/20/2019, in Mesh wl1.1 is backhaul */
		sprintf(command, "wl -i wl1.2 assoclist >> %s", GUEST_ASSOC_LIST);
#else
...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	if ((lease = find_lease_by_chaddr(oldpacket->chaddr)) &&
		(arpping2(lease->yiaddr, server_config.server, server_config.arp, server_config.interface,oldpacket->chaddr) == 1)) {
...
#ifdef BRCM_EZ_MESH_BASE
    #define VENDOR_STR_NETGEAR_SATELLITE "udhcp "VERSION" from NETGEAR MS"
    #define VENDOR_STR VENDOR_STR_NETGEAR_SATELLITE
    char line_buf[1024];
    char filename[64];
    unsigned char *vendor_str;
    if(vendor_str = get_option(oldpacket, DHCP_VENDOR))
    {
	    LOG(LOG_INFO, "DHCP_VENDOR is >%s<", vendor_str);
        /*only save vendor string match to MS60*/
        if(strncmp(vendor_str,VENDOR_STR_NETGEAR_SATELLITE,sizeof(VENDOR_STR_NETGEAR_SATELLITE)-1)==0)
        {
            snprintf(filename, sizeof(filename), "/tmp/udhcpd/%02X%02X%02X%02X%02X%02X",  
                *client_mac, *(client_mac+1), *(client_mac+2), *(client_mac+3), *(client_mac+4), *(client_mac+5));
            if((fp = fopen(filename,"w"))!=NULL)
            {
                snprintf(line_buf, sizeof(line_buf), "%d.%d.%d.%d", 
                    *client_ip, *(client_ip+1), *(client_ip+2), *(client_ip+3));
//                snprintf(line_buf, sizeof(line_buf), "%d.%d.%d.%d %s", 
//                    *client_ip, *(client_ip+1), *(client_ip+2), *(client_ip+3), vendor_str);

                printf("udhcpd write %s to %s\n",line_buf,filename);
                fprintf(fp,"%s\n",line_buf);
                fclose(fp);
            }
        }
