...
{
    //remove all.
    //should access nvram directly.
    printf("%s%d  (dhcpc) should access nvram directly\n",__FUNCTION__,__LINE__); 
    return 0;
}
...
static void change_mode(int new_mode)
{
	...
	{
		/* foxconn added start, zakcer, 09/03/2011 */
		/* Try to fix dhcpc renew fail issue. Seems dhcpc will fail to
		 * recv DHCP ACK in LISTEN_KERNEL mode but OK in LISTEN_RAW mode
		 */
#ifdef WLAN_REPEATER
		if (new_mode == LISTEN_KERNEL)
			new_mode = LISTEN_RAW;
#endif
		/* foxconn added end, zacker, 09/03/2011 */
	    if (listen_mode == LISTEN_NONE)
...
#ifdef ONE_SSID
int get_mac_from_ip(unsigned char *server_id,unsigned char *mac)
{
	  #define NET_ARP_FILE "/proc/net/arp"
    FILE *fp;
    
    memset(mac,0,6);
    fp=fopen(NET_ARP_FILE,"r");
    if(fp)
    {
        while(!feof(fp))
        {
            ...
            if(strstr(buffer,server_id))
            {
                sscanf (buffer,"%*s%*s%*s%s",mac);
                fclose(fp);
                return 1;
...
#define DHCPC_TIMEOUT_RESTART_INTERVAL		10
...
#if defined(_XDSL_PRODUCT) || defined(CONFIG_ORANGE_ISP)
		c = getopt_long(argc, argv, "c:fbH:h:i:m:np:qr:s:o:t:vl", arg_options, &option_index);
#else
		c = getopt_long(argc, argv, "c:fbH:h:i:m:np:qr:s:vl", 
...
#endif
		case 'm':
			strncpy(mac_str, optarg, sizeof(mac_str));
			break;
		case 'v':
    ...
    /* Add option to specify the mac which is used to get IP */
    if (strlen(mac_str) > 0) {
        if (6 == sscanf(mac_str, "%02x:%02x:%02x:%02x:%02x:%02x", 
            ...
            for (c=0; c<6; c++) {
                client_config.arp[c] = (unsigned char)mac_tmp[c];
            }
            //LOG(LOG_INFO, "udhcp client mac %02x:%02x:%02x:%02x:%02x:%02x end", 
	...
	for (;;) {
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
#ifdef WLAN_REPEATER
                /* Foxconn modify start, Max Ding, 03/03/2014 add AP Mode for repeater */
				/* run_on_lan = 1 is AP_MODE */
				//if (1)
				if ((run_on_lan == 0) || (packet_num < 3))
				/* Foxconn modify end, Max Ding, 03/03/2014 */
#else
				if (packet_num < 3)
#endif
				{
...
#ifdef WLAN_REPEATER
					/* foxconn, added by Michael Jiang. */
					/* for getting IP from  rootAP more quickly */
					/* Foxconn modify start, Max Ding, 03/03/2014 add AP Mode for repeater */
					if (run_on_lan) //AP_MODE
						timeout = now + ((packet_num == 2) ? 4 : 2);
					else
						timeout = now + 1;
					/* Foxconn modify end, Max Ding, 03/03/2014 */
#elif (defined NEW_WANDETECT)
			...
			case REQUESTING:
				/* Foxconn, Modified by Michael Jiang, from packet_num 3 to 6. */
#ifdef WLAN_REPEATER
				if(packet_num < 6) {/* double waiting time. */
#else
				if (packet_num < 3) {
#endif
					/* send request packet */
...
#ifdef ONE_SSID
					if(temp=get_option(&packet.data, DHCP_ONE_SSID))
					{
printf("Get ONE_SSID option\n");
						char md5_calculate[34];
						char client_id[10];
int idx;						
  		      ...
  		      {
  		      	memcpy(client_id,client_config.clientid,9);
  		      	client_id[3]=0x02;
  		      	client_id[4]=0x0F;
  		      	client_id[5]=0xB5;
	            option43_md5_calculate(client_config.hostname,client_id,md5_calculate);

  		      	
							if(memcmp(&md5_calculate[OPT_DATA],temp,32)==0)
							{
								/* the return value of option 43 is correct, so record the server ip and mac into a file */
								#define ROOT_AP_INFO_FILE "/tmp/root_ap_info"
								FILE *fp;
								char server_id[32],mac[32];
																
								temp=get_option(&packet.data,DHCP_SERVER_ID);
                                if (temp) {
                                    sprintf(server_id,"%d.%d.%d.%d",temp[0],temp[1],temp[2],temp[3]);
                                    
                                    printf("update file server id=%s\n",server_id);
                                    get_mac_from_ip(server_id,mac);
                                    
                                    fp=fopen(ROOT_AP_INFO_FILE,"w+");
                                    if(fp)
                                    {
                                        fprintf(fp,"%s %s\n",server_id,mac);
                                        fclose(fp);
