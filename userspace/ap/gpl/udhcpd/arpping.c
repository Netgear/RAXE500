...
int arpping2(u_int32_t yiaddr, u_int32_t ip, unsigned char *interface_mac, char *interface,char *leased_mac)
{
	...
	memset(&arp, 0, sizeof(arp));
	memcpy(arp.ethhdr.h_dest, MAC_BCAST_ADDR, 6);	/* MAC DA */
	memcpy(arp.ethhdr.h_source, interface_mac, 6);		/* MAC SA */
	arp.ethhdr.h_proto = htons(ETH_P_ARP);		/* protocol type (Ethernet) */
	arp.htype = htons(ARPHRD_ETHER);		/* hardware type */
	arp.ptype = htons(ETH_P_IP);			/* protocol type (ARP message) */
	arp.hlen = 6;					/* hardware address length */
	...
	*((u_int *) arp.sInaddr) = ip;			/* source IP address */
	...
	memcpy(arp.sHaddr, interface_mac, 6);			/* source hardware address */
    /* Foxconn modified pling start 03/0/9/2006: avoid alignment problem */
	/* *((u_int *) arp.tInaddr) = yiaddr;*/		/* target IP address */
    memcpy(arp.tInaddr, (char *)&yiaddr, 4);
    /* Foxconn modified pling end 03/0/9/2006 */
	...
	while (timeout > 0) {
		...
		if (select(s + 1, &fdset, (fd_set *) NULL, (fd_set *) NULL, &tm) < 0) {
			...
			if (arp.operation == htons(ARPOP_REPLY) && 
			    bcmp(arp.tHaddr, interface_mac, 6) == 0 && 
			    *((u_int *) arp.sInaddr) == yiaddr) {
				DEBUG(LOG_INFO, "Valid arp reply receved for this address");
				if(bcmp(arp.sHaddr,leased_mac,6))
                                {
				DEBUG(LOG_INFO, "arp  receved from other client useing  this address");
                                    rv = 0;
				    break;
                                }
			}
