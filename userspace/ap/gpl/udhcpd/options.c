...
/*added by antony start 2017/04/06 support of one_ssid*/
#ifdef ONE_SSID
	{"md5",	OPTION_STRING | OPTION_LIST | OPTION_REQ,	0x2b},
#endif
/*added by antony end 2017/04/06 */
...
#ifdef ONE_SSID
int option43_md5_calculate(unsigned char *hostname,unsigned char *client_id,unsigned char *result)
{
    unsigned char digest[16],md5[34],plaintext[1024];;
    MD5_CTX one_ssid_md5;
    int plaintext_len,idx;

    memset(plaintext,0,sizeof(plaintext));
    memcpy(plaintext,hostname,hostname[OPT_LEN]+2);  
    memcpy(&plaintext[hostname[OPT_LEN]+2],client_id,client_id[OPT_LEN]+2);  
    plaintext_len=hostname[OPT_LEN]+2+client_id[OPT_LEN]+2;



    MD5Init(&one_ssid_md5);
    MD5Update(&one_ssid_md5,plaintext,plaintext_len);

    MD5Final(digest,&one_ssid_md5);

#define OPTION_ONE_SSID_DATA_LENGTH 32

    result[0]=DHCP_ONE_SSID;
    result[1]=OPTION_ONE_SSID_DATA_LENGTH;
    for(idx=0;idx<=15;idx++)
    {
        result[2*idx+OPT_DATA]=digest[idx]/16;
        result[2*idx+1+OPT_DATA]=digest[idx]%16;

        if(result[2*idx+OPT_DATA]>=10)
            result[2*idx+OPT_DATA] += 'a' - 10;
        else
            result[2*idx+OPT_DATA] += '0';

        if(result[2*idx+1+OPT_DATA]>=10)
            result[2*idx+1+OPT_DATA] += 'a' - 10;
        else
            result[2*idx+1+OPT_DATA] += '0';
    } 
