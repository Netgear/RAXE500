...
typedef struct
{
    ...
    int ftp_num; //The number of ftp connections.
}CON_STATISTIC;
*/
...
# endif
    ...
    printf("-----> %s\n", __FUNCTION__);
#endif
    if (access("/tmp/shm_id2", F_OK) == 0)
    {
        shmid_fp = fopen("/tmp/shm_id2", "r");
        if(shmid_fp != NULL){
            fscanf(shmid_fp, "%d", &segment_id);
#ifdef DEBUG
            printf("segment_id: %d , in /tmp/shm_id2. \n", segment_id);
#endif
    }
        else
        {
            printf("/tmp/shm_id2 open failed\n");
            exit(1);
        }
    }
    else
    {
        key_t unique_key = 112233;
        /* Create a new shared_memory ID. */
        segment_id = shmget (IPC_PRIVATE, shared_segment_size,
                IPC_CREAT | IPC_EXCL | S_IRUSR | S_IWUSR);
        /* write shm_ID back */
        shmid_fp = fopen("/tmp/shm_id2", "w");
        if((shmid_fp != NULL) && (segment_id != -1))
        {
            printf("shm ID: %d\n", segment_id);
            fprintf(shmid_fp, "%d", segment_id);
            fclose(shmid_fp);
        }
        else
        {
            printf("/tmp/shm_id2 opened error!\n");
            exit (1);
        }
        con_st = (CON_STATISTIC*)shmat(segment_id, 0, 0);
        
        /* Create a Semaphore. */
        con_st->sem_id = binary_semaphore_allocation (unique_key, IPC_CREAT | IPC_EXCL | 0666);
        if(con_st->sem_id == -1)
        {
            ...
            binary_semaphore_initialize (con_st->sem_id);
        
    }
    /* Do final checking of shared_memory ID if correct! */
    if(con_st == NULL)
        con_st = (CON_STATISTIC*)shmat(segment_id, 0, 0);


    if((segment_id != -1) && (con_st->sem_id != -1))
    {
    con_st->num = 0;
        printf(" Get a correct Segment_ID: %d and semaphore ID:%d\n", 
                                        segment_id, con_st->sem_id);
    } 
    else
    {
        printf(" Get a incorrect Segment_ID: %d and semaphore ID:%d\n",
                                                segment_id, con_st->sem_id);
        printf("something wrong in getting shm_id. \n");
        exit (1);
    }
