...
typedef struct UserIpAddr
{
    BOOL addr_type;/* 0: ipv4, 1: ipv6 */
...
connection *connection_accept(server *srv, server_socket *srv_socket) {
	...
	if (-1 == (cnt = accept(srv_socket->fd, (struct sockaddr *) &cnt_addr, &cnt_len))) {
		...
		connection *con;
        UserIpAddr_s common_addr;
        struct sockaddr_in * their_addr;
        char ipv4_addr[64];
        //FILE *fp;

        
        their_addr = (struct sockaddr_in *) &cnt_addr;
        common_addr.addr4.s_addr = their_addr->sin_addr.s_addr;
        strcpy(ipv4_addr, inet_ntoa(common_addr.addr4));
        if(con_st != NULL && strcmp(ipv4_addr, "127.0.0.1"))
        {
            binary_semaphore_wait(con_st->sem_id);
            con_st->num = 0;
            con_st->ftp_num = 0;
            con_st->wan_ftp_num = 0;
            /*if(fp = fopen("/tmp/cur_access_user_ip", "w"))
            {
                fprintf(fp, "%s", ipv4_addr);
                fclose(fp);
            }*/
