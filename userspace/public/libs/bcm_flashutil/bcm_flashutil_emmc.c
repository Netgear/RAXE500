...
int emmcGetBootPartition(void)
{
    ...
    {
        printf("ERROR!!! Could not open /proc/cmdline\n");
        return(0);
    }

    if(fgets(line, sizeof(line), fp))
    {
        /* Check if we have device-mapper root device */
        if( strstr(line, "root=/dev/dm") ) 
        {
            dev_str = strstr(line, "/dev/mmcblk");
            if( dev_str ) 
            {
                dev_str = strtok(dev_str, " ");
                strcpy(rootfs_devname, dev_str);
                len = strlen(rootfs_devname);
            }
            else
            {
                printf("ERROR!!! Could not detect flash device for mapped root device\n");
                fclose(fp);
                return(0);
            }
        }
...
uint64_t emmcGetAvailLoaderSpace( void ) {
	...
	uint64_t size_bytes = 0;
        ...
        //FIXME: Update to accept image index for both boot partitions
	sprintf(full_part_name, "/dev/mmcblk0boot0");
