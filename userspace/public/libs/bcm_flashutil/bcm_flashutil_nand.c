...
int nandGetBootPartition(void)
{
    ...
    else
    {
        ...
        {
            fprintf(stderr, "ERROR!!! Could not open /proc/cmdline\n");
            return(0);
        }

        if(fgets(line, sizeof(line), fp))
        {
            if( strstr(line, "ubifs"))
                    isUbifsRootfs = 1;

            token = strtok(line, " ");
            while (token != NULL) 
            {
                if( strstr(token, "root="))
                {
                    if( isUbifsRootfs )
                    {
                        sscanf(token,"root=ubi:rootfs" "%d", &ret);
                    }
                    else
                    {
                        sscanf(token,"root=/dev/ubiblock0_" "%d", &ret);
...
unsigned char *nandUpdateSeqNum(unsigned char *imagePtr, int imageSize, int blkLen, int seq, int *found)
{
	...
	seq = (seq + 1) % 999; // set newly written image sequence number one greater than other image
        setImgSeqNum(nandGetBootPartition() == 1 ? 2 : 1, seq);
...
{ 
    ...
    erase_blk_size = getSysfsBytes(sysfs_path);
...
uint64_t nandGetAvailLoaderSpace(void) 
{
    ...
    char sysfs_path[] = "/sys/class/mtd/mtd0/size";
    loader_size = getSysfsBytes(sysfs_path);
    return loader_size;
}

/* mdev can take upto 2 seconds to create dev_nodes */
#define DEV_NODE_CREATE_TIMEOUT_USEC       3000000       
#define DEV_NODE_CREATE_CHK_INTRVL_USEC    500000       
int nandUbiVolDevNodeExists( char * dev_path )
{
    int dev_node_create_wait_cnt=DEV_NODE_CREATE_TIMEOUT_USEC;
     ...
     */
    while( dev_node_create_wait_cnt > 0 )
    {
        /* If node doesnt exist we sleep and check again */
        if( access( dev_path, F_OK ) ) 
        {
            usleep(DEV_NODE_CREATE_CHK_INTRVL_USEC);
            dev_node_create_wait_cnt -= DEV_NODE_CREATE_CHK_INTRVL_USEC;
        }
        else
            break;
    }

    return (dev_node_create_wait_cnt>0);
}
...
int setNandMetadata( char * data, int size, int mdata_idx )
{
    ...
    sprintf(cmd, "/dev/ubi0_%d", mdata_idx) ;
    if( access( cmd, F_OK ) ) 
    {
        sprintf(cmd, "ubimkvol /dev/ubi0 -s %d -n %d -N metadata%d --type=static", size, mdata_idx, mdata_idx);
        flashutil_debug("=> %s\n", cmd);
        rc = system(cmd);
        if( rc )
            goto err_exit;
    ...
    {
        sprintf(cmd, "ubiupdatevol /dev/ubi0_%d /var/mdata.tmp", mdata_idx);
        flashutil_debug("=> %s\n", cmd);
        rc = system(cmd);
