...
static UBOOL8 isStringBuf(const char *buf, UINT32 len)
{
   ...
   if (buf[len-1] != '\0') // no null termination, not a string buf
      return FALSE;

   for (i=0; i < len-1; i++)
      if (buf[i] == '\0' || !isascii(buf[i]))  // premature null, or not ascii
         return FALSE;
...
static void dumpAllocRec(AllocRecord *allocRec, UBOOL8 dumpStackTrace)
{
   ...
   printf("%06u.%06u addr=%p sz=%05d [",
          allocRec->tms.sec, allocRec->tms.nsec/1000,
          allocRec->bufAddr, allocRec->userSize);
   ...
   if (isStringBuf((const char *) allocRec->bufAddr, allocRec->userSize))
   {
      UINT32 i;
      // Dump as string buf
      for (i=0; i < allocRec->userSize-1; i++)
