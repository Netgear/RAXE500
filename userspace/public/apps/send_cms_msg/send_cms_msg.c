...
{
   printf("usage: send_cms_msg [-v] [-r|-o|-e] [-c component_name] [-d wordData] [-t timeout] [-p pid] message dest_eid\n");
   exit(-1);
}
...
int main(int argc, char **argv)
{
   ...
   while ((c = getopt(argc, argv, "c:d:vroep:t:")) != -1)
   {
      switch(c) {
      case 'v':
         verbose = 1;
   ...
   ret = cmsMsg_initOnBus(EID_SEND_CMS_MSG, 0, busName, &msgHandle);
