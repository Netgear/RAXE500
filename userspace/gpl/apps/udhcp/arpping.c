...
int arpping(u_int32_t yiaddr, u_int32_t ip, char *interface, u_int8_t *chaddr) {
	...
	return arpCheck(yiaddr, &ifbuf, 50, chaddr); //Waiting ARP reply for 50 ms
}

#define SET_MS_TVAL(tv,mstime) do{tv.tv_sec = (mstime) / 1000 ;tv.tv_usec = ((mstime) % 1000) * 1000;}while(0)

int arpCheck(u_long inaddr, struct ifinfo *ifbuf, long timeout_ms, u_int8_t *chaddr)  {
	int				s;			/* socket */
	int				rv;			/* return value */
	...
	while ( timeout_ms > 0 ) {
		...
		if ( select(s+1, &fdset, (fd_set *)NULL, (fd_set *)NULL, &tm) < 0 ) {
			...
			if(arp.operation == htons(ARPOP_REPLY) && 
			   ...
			   *((u_int *)arp.sInaddr) == inaddr ) {
				...
				if(memcmp(arp.sHaddr, chaddr, 6)){ // no reply by the owner itself
				    rv = 0;
				}
				break;
			}
		...
		timeout_ms -= ((now.tv_sec-prevTime.tv_sec)*1000) + (now.tv_usec-prevTime.tv_usec)/1000;
		gettimeofday(&prevTime,NULL);        
	}
	close(s);
