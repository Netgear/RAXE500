...
void l2tp_load_config_file(void)
{
    ...
    if ((strlen(iremote) > 0) && (strlen(iuserid) > 0) && 
        (strlen(ipassword) > 0) && (strlen(devname) > 0))
    {
    	// ppp
    	...
    	OPTSTRING(session.interface_name) = strdup(devname);
