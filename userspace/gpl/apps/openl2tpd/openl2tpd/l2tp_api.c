...
static SVCXPRT	*l2tp_rpc_unix_xprt;
#endif
...
#ifdef BRCM_L2TP_SUPPORT 
    unlink(L2TP_RPC_SERVER_ADDR);
    
    l2tp_rpc_unix_xprt = svcunix_create(RPC_ANYSOCK, 0, 0, L2TP_RPC_SERVER_ADDR);
    if (l2tp_rpc_unix_xprt == NULL) {
        l2tp_log(LOG_ERR,"unable to create unix service with RPC\n");
        exit(1);
    }
    
    result = svc_register(l2tp_rpc_unix_xprt, L2TP_PROG, L2TP_VERSION, l2tp_prog_1, 0);
    if (result == 0) {  /* UNIX is nice and consistent about error codes ;-) */
        l2tp_log(LOG_ERR,"unable to register unix RPC program\n");
        exit(1);
    }
   
    result = usl_fd_add_fd(l2tp_rpc_unix_xprt->xp_sock, l2tp_api_rpc_msg, NULL);
    if (result < 0) {
        l2tp_log(LOG_ERR, "unable to register unix RPC handler\n");
        exit(1);
    }
#else
...
void l2tp_api_cleanup(void)
{
    ...
    if (l2tp_rpc_unix_xprt != NULL) {
        svc_unregister(L2TP_PROG, L2TP_VERSION);
        xprt_unregister(l2tp_rpc_unix_xprt);
		svc_destroy(l2tp_rpc_unix_xprt);
    }
