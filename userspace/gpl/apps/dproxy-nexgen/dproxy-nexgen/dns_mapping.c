                                   ...
                                   char *dns1, int *proto)
{
   ...
   if (queryType == AAA)
   {
      ...
      else if ((curr->ipv4_dns1) && cmsUtl_strcmp(curr->ipv4_dns1, "0.0.0.0"))
      {
   ...
   else
   {
       ...
       * address if available.
       */
      if ((curr->ipv4_dns1) && cmsUtl_strcmp(curr->ipv4_dns1, "0.0.0.0"))
      {
         strcpy(dns1, curr->ipv4_dns1);
         *proto = AF_INET;
         found = TRUE;
...
void dns_mapping_do_dns_swap(char *dnsUsed)
{
   ...
   while (curr && !swapDone)
   {
      if (curr->ipv4_dns1 && (curr->ipv4_dns2 && cmsUtl_strcmp(curr->ipv4_dns2, "0.0.0.0")) && !cmsUtl_strcmp(dnsUsed, curr->ipv4_dns1))
      {
