...
sub new {
    ...
    $self->check_args(); 
    if ($self->{args}->{chip} =~ m/(63138|63148|63268|631181)/g) {
       $self->{args}->{arch} = "GEN2";
    } 
    #print_hash($args);
    return $self;
...
sub modeEligible {
    ...
    return ( defined $opt->{'encrypt'} )
      ...
      : ( $arg->{'sec_mode'} eq 'UNSEC' ? 1 : ($arg->{sec_mode} eq 'MFG' && $opt->{unsec_eligible}? 3:6) );
}
...
sub sbi_header {
    ...
    if ($arg->{arch} eq 'GEN2') {
        $self->{lcot} = $self->_2var(_E($self->_2path('cot')));
	...
	if ($tmp =~ m/.*opt<.*unsec_eligible.*>/) {
        	print "------------- Will set unsec_eligible  ---------------------\n";
        	$arg->{sec_opt}->{unsec_eligible} = 'Y';
        }
