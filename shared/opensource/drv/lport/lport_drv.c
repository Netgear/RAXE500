...
/* XXX: should comment out once LEDS remove BP support: #ifdef BP_SUPPORT */
#define BP_SUPPORT
#ifdef BP_SUPPORT
...
static int rgmii_init(lport_init_s *init_params)
{
    ...
    for (i = LPORT_FIRST_RGMII_PORT; i < LPORT_NUM_OF_PORTS; i++)
    {
        ...
        if (!init_params->has_rgmii_cfg)
        {
            has_phy = emac_info->sw.phy_id[i] & PHY_EXTERNAL; 
            ...
            rgmii_cfg.is_1p8v = IsRGMII_1P8V(emac_info->sw.phy_id[i]);
        }
        else
#endif
        {
