...
int phy_dsl_serdes_init(phy_dev_t *phy_dev)
{
    ...
    {
        printk("******** ERROR: TWo many Serdeses number to support.\n");
        BUG();
    }
    serdes[serdes_index++] = phy_serdes;
    phy_serdes->hw_speed = -1;
    phy_serdes->highest_speed = PHY_SPEED_UNKNOWN;
    phy_serdes->lowest_speed = PHY_SPEED_HIGHEST_SUPPORT;
    phy_dev->inter_phy_types = phy_serdes->inter_phy_types;
    for (speed = PHY_SPEED_HIGHEST_SUPPORT; speed != PHY_SPEED_AUTO; speed--)
    {
        speed_caps = phy_speed_to_caps(speed, PHY_DUPLEX_FULL);
        if (!(phy_serdes->speed_caps & speed_caps))
            continue;

        if (phy_serdes->highest_speed < speed)
            phy_serdes->highest_speed = speed;

        if (phy_serdes->lowest_speed > speed)
            phy_serdes->lowest_speed = speed;
    }
    phy_serdes->highest_speed_cap = phy_speed_to_caps(phy_serdes->highest_speed, PHY_DUPLEX_FULL);
    phy_serdes->lowest_speed_cap = phy_speed_to_caps(phy_serdes->lowest_speed, PHY_DUPLEX_FULL);
    ...
    if ((phy_dev->cascade_next == NULL))
    {
        ...
        phy_drv_dsl_i2c_create(phy_dev);
    }
#endif
...
char *dsl_serdes_get_phy_name(phy_dev_t *phy_dev)
{
    ...
    n += snprintf(buf+n, sz-n, "%s#%d", phy_dev->phy_drv->name, phy_dev->core_index);
    return buf;
}
...
int dsl_serdes_single_speed_set(phy_dev_t *phy_dev, phy_speed_t speed, phy_duplex_t duplex)
{
    phy_serdes_t *phy_serdes = phy_dev->priv;
    int rc = 0;
    int speed_caps, cfg_speed_cap;

    if (!phy_serdes || !phy_serdes->inited)
        return 0;

    phy_dev_caps_get(phy_dev, CAPS_TYPE_SUPPORTED, &speed_caps);
    cfg_speed_cap = phy_speed_to_caps(speed, PHY_DUPLEX_FULL);
    if (!(cfg_speed_cap & speed_caps)) {
...
static void dsl_serdes_speed_detect(phy_dev_t *phy_dev)
{
    ...
    for (speed = phy_serdes->highest_speed; speed >= phy_serdes->lowest_speed; speed--)
    {
...
NoLinkUp:
    ...
    */
    ...
    {
        phy_dev->current_inter_phy_type = phy_get_best_inter_phy_configure_type(phy_dev,
            phy_dev->configured_inter_phy_types, phy_serdes->highest_speed);
        dsl_serdes_single_speed_set(phy_dev, phy_serdes->highest_speed, PHY_DUPLEX_FULL);
    }
end:
    retry = 0;
}
...
int dsl_serdes_caps_get(phy_dev_t *phy_dev, int caps_type, uint32_t *caps)
{
    ...
    else if (caps_type == CAPS_TYPE_ADVERTISE)
    {
        if (phy_dsl_priv->config_speed == PHY_SPEED_AUTO)
            ...
            *caps = phy_speed_to_caps(phy_dsl_priv->config_speed, PHY_DUPLEX_FULL);
    }
    else
        *caps = 0;
    return 0;
...
int dsl_serdes_cfg_speed_set(phy_dev_t *phy_dev, phy_speed_t speed, phy_duplex_t duplex)
{
    ...
    if(speed == PHY_SPEED_AUTO)
        phy_serdes->current_speed = phy_serdes->highest_speed;
    else
        phy_serdes->current_speed = speed;

    phy_dev->current_inter_phy_type = phy_get_best_inter_phy_configure_type(phy_dev,
            phy_dev->configured_inter_phy_types, phy_serdes->current_speed);
    ...
    if (phy_serdes->sfp_module_type == SFP_FIXED_PHY)
    {
        if (IS_USER_CONFIG())
            ...
            if(copper_phy->link)
            {
                ...
                if (!phy_dev->link)
                    printk("Warning: Serdes link does not go up following external copper PHY.\n");
            }
        }
    }
    else
...
static int dsl_sfp_module_detect(phy_dev_t *phy_dev)
{
    ...
    if(phy_serdes->sfp_status < SFP_MODULE_IN)
    {
        ...
        if (phy_i2c->flag & (PHY_FLAG_COPPER_SFP_TYPE|PHY_FLAG_COPPER_CONFIGURABLE_SFP_TYPE)) //SFP_COPPER
            printk("%dG Sgmii Copper SFP Module is Plugged in at Serdes address %d core %d\n", (speed_caps & PHY_CAP_10000)? 10: 1,
                    phy_dev->addr, phy_serdes->core_num);
        else
            printk("XBase-X SFP Module under Serdes address %d core %d is Plugged in\n",
                    phy_dev->addr, phy_serdes->core_num);
    }
...
int phy_dsl_serdes_read_status(phy_dev_t *phy_dev)
{
    ...
    if (phy_serdes->power_mode > SERDES_NO_POWER_SAVING && phy_serdes->sfp_status < SFP_LINK_UP)
        dsl_powerstandby_serdes(phy_dev);
...
sfp_module_in:
            ...
            if(phy_serdes->sfp_status < SFP_MODULE_IN)
            {
                cascade_phy_set_common_inter_types(phy_dev);
                phy_dsl_set_configured_types(phy_dev);
                dsl_powerdown_serdes(phy_dev);  /* Power down and up Serdes to reset Serdes Status */
                dsl_powerup_serdes(phy_dev);
                phy_dev->current_inter_phy_type = phy_next->current_inter_phy_type =
                    phy_get_best_inter_phy_configure_type(phy_dev,
                        phy_dev->configured_inter_phy_types, phy_serdes->config_speed);
            ...
            if(phy_serdes->sfp_status == SFP_LINK_UP)
            {
                ...
                if(!phy_dev->link)
                {
                    ...
                    if(phy_serdes->config_speed == PHY_SPEED_AUTO)
                    {
                        ...
                        phy_dev->current_inter_phy_type = phy_get_best_inter_phy_configure_type(phy_dev,
                                phy_dev->configured_inter_phy_types, highest_speed);
                        dsl_serdes_single_speed_set(phy_dev, highest_speed, PHY_DUPLEX_FULL);
                    }
                    goto sfp_module_in;
                }
            }
