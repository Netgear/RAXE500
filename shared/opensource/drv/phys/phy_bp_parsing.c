...
static int bp_parse_caps_mask(phy_mii_type_t mii_type)
{
...
phy_dev_t *bp_parse_phy_dev(const EMAC_PORT_INFO *port_info)
{
    ...
    phy_dev->caps_mask = bp_parse_caps_mask(phy_dev->mii_type);
    ...
    bus_type = bp_parse_bus_type(port_info);
    phy_dev->bus_drv = bus_drv_get(bus_type);
