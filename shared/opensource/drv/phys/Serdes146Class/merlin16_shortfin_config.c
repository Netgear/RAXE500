...
#define print_log(fmt, args...) do { \
    phy_serdes_t *phy_serdes = phy_drv_serdes146_get_serdes(CoreNum); \
    if (log_level >= 2 && !phy_serdes->inited) printk(fmt, ##args); \
} while(0)
...
int merlin_pmi_write16_delay(uint32_t CoreNum, uint32_t LANE, uint32_t DEV_ADDR, uint32_t REG_ADDR, uint16_t DATA, uint16_t MASK, bool delay_acked)
{
    ...
    if(0 && parse_sim_opts("-d VERBOSE")) {
        print_log("... MerlinSupport::merlin_pmi_write16():Writing 0x%08x to register address 0x%08x\n", acc_addr_data, acc_addr_addr);
        print_log("... MerlinSupport::merlin_pmi_write16():Writing 0x%08x to register address 0x%08x\n", acc_mask_data, acc_mask_addr);
        print_log("... MerlinSupport::merlin_pmi_write16():Writing 0x%08x to register address 0x%08x\n", acc_ctrl_data, acc_ctrl_addr);
    }
    ...
    if (delay_acked) {
        ...
        // print_log("... MerlinSupport::merlin_pmi_write16(): PMI is in indirect mode\n");
    }
        ...
        if (watch_dog > 15) {  // should be finished by 1us
            if(parse_sim_opts("-d ML_C_ALLOW_PMI_TIMEOUT")) {
                if( (REG_ADDR == 0x0000) && (DATA == 0xdead) && (MASK == 0xffff) ) {
                    // print_log("INFO !!! MerlinSupport::merlin_pmi_write16(): PMI write has time-out as expected, simulation resumes becasue of '-d ML_C_ALLOW_PMI_TIMEOUT' is specified\n");
                } else {
                    error_log("INFO !!! MerlinSupport::merlin_pmi_write16(): PMI write has time-out\n");
                }
                busy = 0x0;
            } else {
                if ((delayed_ack == 0x0) && (acc_ctrl_read == 0xeeeeeeee) && (REG_ADDR == 0xc30b)) {  //rbus error handling for chip_test/r2pmi_lp_bridge_test.cc
                    // print_log("INFO !!! MerlinSupport::merlin_pmi_write16(): PMI write has time-out as expected, simulation resumed for r2pmi_lp_bridge_test\n");
                    busy = 0;
                } else {
                    fatal_log("ERROR !!! MerlinSupport::merlin_pmi_write16():PMI transaction write-to Merlin Core #%d has timed-out\n", CoreNum);
                }
            }
        } else {
            if(0 && parse_sim_opts("-d VERBOSE")) { print_log("... MerlinSupport::merlin_pmi_write16(): acc_ctrl = 0x%x @watch dog count = 0x%08d\n", acc_ctrl_read, watch_dog); }
        }
    }

    if(0 && parse_sim_opts("-d VERBOSE")) { print_log("INFO MerlinSupport::merlin_pmi_write16(): PMI write done\n"); };
    return 0;
}

uint32 merlin_pmi_read16_delay(uint32 CoreNum, uint32 LANE, uint32 DEV_ADDR, uint32 REG_ADDR, bool DELAY_ACKED)
{
    ...
    if(0 && parse_sim_opts("-d VERBOSE")) {
        print_log("... MerlinSupport::merlin_pmi_read16():Writing 0x%08x to register address 0x%08x\n", acc_addr_data, acc_addr_addr);
        print_log("... MerlinSupport::merlin_pmi_read16():Writing 0x%08x to register address 0x%08x\n", acc_mask_data, acc_mask_addr);
        print_log("... MerlinSupport::merlin_pmi_read16():Writing 0x%08x to register address 0x%08x\n", acc_ctrl_data, acc_ctrl_addr);
    }
    ...
    if (DELAY_ACKED) {
        ...
        busy = 0x1;  //true; polling
        // print_log("... MerlinSupport::merlin_pmi_read16(): PMI is in indirect mode\n");
    }
    ...
    while (busy) {
        ...
        if (watch_dog > 15) {  // should be finished by 1us
            if(parse_sim_opts("-d ML_C_ALLOW_PMI_TIMEOUT")) {
                error_log("INFO !!! MerlinSupport::merlin_pmi_read16(): PMI read has time-out\n");
            } else {
                if ((delayed_ack == 0x0) && (acc_ctrl_read == 0xeeeeeeee) && (REG_ADDR == 0xc30b)) {  //rbus error handling for chip_test/r2pmi_lp_bridge_test.cc
                    // print_log("INFO !!! MerlinSupport::merlin_pmi_read16(): PMI write has time-out as expected, simulation resumed for r2pmi_lp_bridge_test\n");
                    busy = 0;
                } else {
                    fatal_log("ERROR !!! MerlinSupport::merlin_pmi_read16():PMI transaction read-from Merlin Core #%d has timed-out\n", CoreNum);
                }
            }
        } else {
            if(0 && parse_sim_opts("-d VERBOSE")) { print_log("... MerlinSupport::merlin_pmi_read16(): watch dog count = 0x%08d\n", watch_dog); }
        }
...
static void merlin_load_firmware (phy_dev_t *phy_dev)
{
    ...
    print_log("MerlinSupport::merlin_load_firmware(): INFO: Start to download firmware to core #%4x\n", CoreNum);
    ...
    if (1) {  //addition to the ucode_pram_load()
        ...
        if ((pmi_rd_data & 0x0001) == 0x0) {  //[0]:micro_ra_initdone
            print_log("MerlinSupport::merlin_load_firmware():  Checking init_done initial value passed...\n");
        } else {
            fatal_log("MerlinSupport::merlin_load_firmware():  Checking init_done initial value failed !!!\n");
        }
        if (parse_sim_opts("-d MERLIN_BC") && (CoreNum<3)) {
    ...
    print_log("MerlinSupport::merlin_load_firmware():  INFO: wait 8192 comclks for ~32KB code ram initialization...\n");
    ...
    while (poll) {
        ...
        if ((pmi_rd_data & 0x0001) == 0x1) {  //[0]:micro_ra_initdone
            print_log("MerlinSupport::merlin_load_firmware():  Checking init_done passed ...\n");
            poll = false;
        } else {
            watch_dog++;
            if (watch_dog > 10) {  //10us
                fatal_log("MerlinSupport::merlin_load_firmware():  Checking init_done failed !!!\n");
            }
        }
        timeout_ns(1000);
    }
    ...
    print_log("MerlinSupport::merlin_load_firmware():  INFO: wait 1024 comclks for 3KB data ram initialization...\n");
    timeout_ns(52428);   //8192*6.4
    ...
    print_log("MerlinSupport::merlin_load_firmware():  begin writing firmware to the program ram ...\n");
    do {   /* ucode_image loaded 16bits at a time */
        if (count % ((ucode_len_padded/80)&(~3)) == 0) printk(KERN_CONT "#");
        if ((count%1000) == 0) {
            //print_log("MerlinSupport::merlin_load_firmware():  DEBUG INFO: writing bytes >%d to program ram...\n", count);
        }
...
log_level = 2;
    printk("Done.\n");
    print_log("MerlinSupport::merlin_load_firmware():  end writing firmware to the program ram ...\n");
...
static void merlin_core_init(uint32 CoreNum, uint32 PRTAD, uint32 LN_OFFSET)
{
    /*
    ...
    // PowerMgrBlk * eth_phy_top_blk = pChipEnv->pwr_mgr->get_blk(PowerMgr::BLKTYPE_ETH);
    reg_def = 0x0 << MERLIN_SERDES_CTRL_LN_OFFSET_OFFSET |    //default to 0;
        PRTAD << MERLIN_SERDES_CTRL_PRTAD_OFFSET |
        0x0 << MERLIN_SERDES_CTRL_REFSEL_OFFSET |       //default to internal CML;
        0x1 << MERLIN_SERDES_CTRL_RESET_OFFSET |        //default to 1; i.e. in reset
        0x1 << MERLIN_SERDES_CTRL_REFCLK_RESET_OFFSET | //default to 1; i.e. in reset
        0x1 << MERLIN_SERDES_CTRL_IDDQ_OFFSET;          //default to 1; i.e. in iddq

    if (parse_sim_opts("-d ML_EXT_REFCLK")) {
        REFSEL = 0x3;
        print_log("INFO MerlinSupport::merlin_core_init(): Disable Analog PLL VCO CHECK for ML_EXT_REFCLK\n");
    }

    print_log("INFO MerlinSupport::merlin_core_init(): START powering up Merlin Core #%d with PRTAD = %d, ln_offset_stap = %d\n", CoreNum, PRTAD, LN_OFFSET);
    print_log("INFO MerlinSupport::merlin_core_init(): Disable IDDQ\n");

    //MDIO phy address
    wr_data = reg_def & (~(MERLIN_SERDES_CTRL_PRTAD_MASK << MERLIN_SERDES_CTRL_PRTAD_OFFSET) | (PRTAD << MERLIN_SERDES_CTRL_PRTAD_OFFSET));
    //LANE address
    wr_data = wr_data & (~(MERLIN_SERDES_CTRL_LN_OFFSET_MASK << MERLIN_SERDES_CTRL_LN_OFFSET_OFFSET) | (LN_OFFSET << MERLIN_SERDES_CTRL_LN_OFFSET_OFFSET));
    //Ref. clock select
    wr_data = wr_data & (~(MERLIN_SERDES_CTRL_REFSEL_MASK << MERLIN_SERDES_CTRL_REFSEL_OFFSET) | ( REFSEL << MERLIN_SERDES_CTRL_REFSEL_OFFSET));
    //clear IDDQ
    wr_data = wr_data & (~(MERLIN_SERDES_CTRL_IDDQ_MASK << MERLIN_SERDES_CTRL_IDDQ_OFFSET));
    ...
    print_log("INFO MerlinSupport::merlin_core_init(): Provide a delay of 1ms after IDDQ is de-asserted\n");
    //timeout_ns(10000);
    timeout_ns(1000);  //shorten the simulation time
    print_log("INFO MerlinSupport::merlin_core_init(): Disable serdes_reset and refclk_reset\n");
    //clear serdes reset
    wr_data = wr_data & ~(MERLIN_SERDES_CTRL_RESET_MASK << MERLIN_SERDES_CTRL_RESET_OFFSET);
    //clear ref. clock reset
    wr_data = wr_data & ~(MERLIN_SERDES_CTRL_REFCLK_RESET_MASK << MERLIN_SERDES_CTRL_REFCLK_RESET_OFFSET);
    ...
    if (parse_sim_opts("-d ML_EXT_REFCLK")) {
        print_log("INFO MerlinSupport::merlin_core_init(): Enable Analog PLL VCO CHECK for ML_EXT_REFCLK\n");
    }

    print_log("INFO MerlinSupport::merlin_core_init(): END powering up Merlin Core #%d with PRTAD = %d, ln_offset_stap = %d\n", CoreNum, PRTAD, LN_OFFSET);
...
static void merlin_wait_uc_active (uint32 CoreNum)
{
    ...
    //delay
    print_log("MerlinSupport::merlin_wait_uc_active():    wait 50us comclks for micro to be up...\n");
    ...
    if ((pmi_rd_data & 0x8000) == 0x8000) { //uc_active@[15]
        print_log("MerlinSupport::merlin_wait_uc_active():  Checking uc_active passed ...\n");
    } else {
        fatal_log("MerlinSupport::merlin_wait_uc_active():  Checking uc_active failed !!!\n");
    }
    ...
    if ((pmi_rd_data & 0x0080) == 0x0080) { //uc_dsc_ready_for_cmd@[7]
        if ((pmi_rd_data & 0x0040) == 0x0040) { //bit 6 is uc_dsc_error_found
            fatal_log("MerlinSupport::merlin_wait_uc_active():  us_dsc_ready for cmd is set and uc_dsc_error is also set !!!\n");
        } else {
            print_log("MerlinSupport::merlin_wait_uc_active():  micro is ready for command ...\n");
        }
    } else {
        fatal_log("MerlinSupport::merlin_wait_uc_active():  uc_dsc_ready_for_cmd is not set !!!\n");
    }
...
static void merlin_mdio_cl45_wr(uint32 PHYAD, uint32 DEVAD, uint32 REG_ADDR, uint16 DATA)
{
    ...
    // print_log("MerlinSupport::merlin_mdio_cl45_wr(): Writing to phy_addr=0x%02x dev_addr=0x%02x reg_addr=0x%04x data=0x%04x\n", PHYAD, DEVAD, REG_ADDR, DATA);
    ...
    if (mdio_err) {
        if(parse_sim_opts("-d ML_C_ALLOW_MDIO_ERR"))
            print_log2("INFO  !!! MerlinSupport::merlin_mdio_cl45_wr(): MDIO fail bit is set with phy_addr=0x%02x dev_addr=0x%02x reg_addr=0x%04x data=0x%04x\n", PHYAD, DEVAD, REG_ADDR, DATA);
        else
            fatal_log("ERROR !!! MerlinSupport::merlin_mdio_cl45_wr(): MDIO fail bit is set with phy_addr=0x%02x dev_addr=0x%02x reg_addr=0x%04x data=0x%04x\n", PHYAD, DEVAD, REG_ADDR, DATA);
    }
...
static uint32 merlin_mdio_cl45_rd(uint32 PHYAD, uint32 DEVAD, uint32 REG_ADDR)
{
    ...
    host_reg_write(MDIO_CFG,0x40);  //set to Clause 45
    // print_log("MerlinSupport::merlin_mdio_cl45_rd(): Reading from phy_addr=0x%02x dev_addr=0x%02x reg_addr=0x%04x \n", PHYAD, DEVAD, REG_ADDR);
    ...
    if (mdio_err) {
        if(parse_sim_opts("-d ML_C_ALLOW_MDIO_ERR"))
            print_log2("INFO  !!! MerlinSupport::merlin_mdio_cl45_rd(): MDIO fail bit is detected with phy_addr=0x%02x dev_addr=0x%02x reg_addr=0x%04x\n", PHYAD, DEVAD, REG_ADDR);
        else
            fatal_log("ERROR !!! MerlinSupport::merlin_mdio_cl45_rd(): MDIO fail bit is detected with phy_addr=0x%02x dev_addr=0x%02x reg_addr=0x%04x\n", PHYAD, DEVAD, REG_ADDR);
    }
    ...
    if (mdio_err) {
        if(parse_sim_opts("-d ML_C_ALLOW_MDIO_ERR"))
            print_log2("INFO  !!! MerlinSupport::merlin_mdio_cl45_rd(): MDIO fail bit is set with phy_addr=0x%02x dev_addr=0x%02x reg_addr=0x%04x\n", PHYAD, DEVAD, REG_ADDR);
        else
            fatal_log("ERROR !!! MerlinSupport::merlin_mdio_cl45_rd(): MDIO fail bit is set with phy_addr=0x%02x dev_addr=0x%02x reg_addr=0x%04x\n", PHYAD, DEVAD, REG_ADDR);
    }
...
{
    uint16 rmw_data;
    rmw_data = merlin_mdio_cl45_rd(HOST_MDIO_PHY_ADDR(CoreNum), DEV_ADDR, REG_ADDR);
    rmw_data = rmw_data & 0xffff;
    MASK = MASK & 0xffff;
    DATA = DATA &0xffff;
    rmw_data = ((rmw_data & MASK) | (DATA & ~MASK));
    merlin_mdio_cl45_wr(HOST_MDIO_PHY_ADDR(CoreNum),  DEV_ADDR, REG_ADDR,  rmw_data);
}
...
#define PRINT_DUPLICATED_REG_SAVE 0
    ...
    {
        printk("Error: Merline register saving array overflow\n");
        BUG();
    }
...
void merlin_reg_prog(prog_seq_tbl *prog_seq_tbl_ptr, uint32 CoreNum, uint32 PortNum)
{
    ...
    for(;strcmp(prog_seq_tbl_ptr->reg_desc, ""); prog_seq_tbl_ptr++) {
        if(parse_sim_opts("-d VERBOSE")) {
            print_log("MerlinSupport::merlin_reg_prog():%s\n", prog_seq_tbl_ptr->reg_desc);
        }
        ...
        if (!strcmp(prog_seq_tbl_ptr->reg_desc, "timeout_100ns")) {
            ...
            print_log("MerlinSupport::merlin_reg_prog(timeout_100ns): %d loops \n", iter);
            print_log("BEGIN : \n");
            for (i=0; i<iter;i++) {
                timeout_ns(100);
            }
...
static void merlin_cfg_core_ram_var (uint32 CoreNum, uint16_t vco_rate)
{
    ...
    print_log("MerlinSupport::merlin_cfg_core_ram_var():  program core_config_word 0x%x to ram address 0x%x ...\n", core_config_word, core_var_ram_base);
...
static void merlin_reset_datapath_core (uint32 CoreNum)
{
    ...
    print_log("MerlinSupport::merlin_reset_datapath_core(): Datapath Reset (core) in progress \n");
...
static void merlin_lane_config_speed(uint32 CoreNum, uint32 PortNum, uint32 LNK_SPD, uint32 an_enabled)
{
    if (an_enabled)
    {
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9107, 0x0000 , 0x7fff );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9250, 0x029a , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9251, 0x029a , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9252, 0xa000 , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9253, 0x10ee , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9254, 0xa000 , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9255, 0x0bb8 , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9256, 0x1b00 , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9257, 0x8235 , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x9258, 0x8235 , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x925a, 0x8235 , 0x0000 );
		merlin_pmi_write16 (CoreNum, 0x0, 0x3, 0x925c, 0x000f , 0x0000 );
	}
    /*
       timeout_ns(100); //delay for simulation debug only
    //---- for simulation speed up -----

    print_log("MerlinSupport::merlin_lane_config_speed(): Core #%d Lane #%d PMD Lock Speed Up programming \n", CoreNum, PortNum);
    ...
    print_log("MerlinSupport::merlin_lane_config_speed(#%x):  \n",LNK_SPD);
    //CL36
    if (LNK_SPD == MLN_SPD_FORCE_5G) {  //Force CL36 5G
        ...
        fatal_log("ERROR !!! MerlinSupport::merlin_lane_config_speed():Merlin Core #%d lane #%d Mode %x is not supported \n", CoreNum, PortNum, LNK_SPD);
  }
    ...
    print_log("INFO MerlinSupport::merlin_lane_config_speed(): END Merlin core #%d lane #%d Initialization procedure\n", CoreNum, PortNum);
...
void merlin_chk_pll_lock (uint32 CoreNum)
{
    ...
    if (CoreNum < NUMBER_MERLIN_CORES) {
        ...
        fatal_log("ERROR !!! MerlinSupport::merlin_chk_pll_lock():Merlin Core #%d\n is not supported", CoreNum);
    }

    print_log("MerlinSupport::merlin_chk_pll_lock(): Checking Core #%d PLL Lock Status \n", CoreNum);
    watch_dog=0;
    pll_lock = 0;
    while (!pll_lock) {
        rd_data = host_reg_read(rd_addr);
        pll_lock = rd_data & (0x1 << 3);   //[3]: PLL_LOCK
        // print_log("MerlinSupport::merlin_chk_pll_lock(): DEBUG INFO: Core #%d read %x from %x, and PLL Lock Status = %d \n", CoreNum, rd_data, rd_addr, pll_lock);
        msleep(10);
        watch_dog = watch_dog + 1;
        if (watch_dog > 50) {   //should be locked within 50us
            //fatal_log("ERROR !!! MerlinSupport::merlin_chk_pll_lock(): Merlin Core #%d PLL lock checking has timed-out\n", CoreNum);
            fatal_log("ERROR !!! MerlinSupport::merlin_chk_pll_lock(): Merlin Core #%d PLL lock checking has timed-out\n", CoreNum);
            break;
        }
    }
    print_log("MerlinSupport::merlin_chk_pll_lock(): PLL Locked\n");
}
    ...
    if (parse_sim_opts("-d MERLIN_BC") && (CoreNum<3)) {
        ...
        if ((pmi_rd_data0 & 0x0080) == 0x0080) { //uc_dsc_ready_for_cmd@[7]
            if ((pmi_rd_data0 & 0x0040) == 0x0040) { //bit 6 is uc_dsc_error_found
                fatal_log("MerlinSupport::merlin_wait_uc_crc_verify():  us_dsc_ready for cmd is set and uc_dsc_error is also set !!!\n");
            } else {
                print_log("MerlinSupport::merlin_wait_uc_crc_verify():  micro is ready for command ...\n");
            }
        } else {
            fatal_log("MerlinSupport::merlin_wait_uc_crc_verify():  uc_dsc_ready_for_cmd is not set !!!\n");
        }
        ...
        if (reset_state & 0x0007) {
            print_log("MerlinSupport::merlin_wait_uc_crc_verify(): DSC ready for command is not working; SerDes is probably reset!!! reset_state = %x \n", reset_state);
        }
        /* Check if ln_s_rstb is asserted by looking at the reset state, the give a warning and display the commands*/
        ...
        print_log("MerlinSupport::merlin_wait_uc_active():  uCode CRC Verify ...\n");
        ...
        print_log("MerlinSupport::merlin_wait_uc_active():  wait 500us for crc verify to be done ...\n");
        host_reg_write(ETH_PHY_TOP_REG_R2PMI_LP_BCAST_MODE_CNTRL,0x000);
        timeout_ns (1000);  //500us
        ...
        while (polling)
        {
            ...
            { //uc_dsc_ready_for_cmd@[7]
                ...
                {
                    fatal_log("MerlinSupport::merlin_wait_uc_active():  CRC verification failed !!!\n");
                } else {
                    print_log("MerlinSupport::merlin_wait_uc_active():  CORE 0 CRC verification passed ...\n");
                    polling =0;
                }
            }
            else
            {
                ...
                else {polling =0;error_log ("MerlinSupport::merlin_wait_uc_active():  CRC verification timeout ...\n");}
            }
        }
        ...
        while (polling)
        {
            ...
            { //uc_dsc_ready_for_cmd@[7]
                ...
                if (calc_crc != MERLIN16_SHORTFIN_UCODE_IMAGE_CRC)
                {
                    ...
                    print_log("MerlinSupport::merlin_wait_uc_active():  CORE 1 CRC verification passed ...\n");
                    polling =0;
                }
            }
            else
        ...
        while (polling)
        {
            ...
            { //uc_dsc_ready_for_cmd@[7]
                ...
                if (calc_crc != MERLIN16_SHORTFIN_UCODE_IMAGE_CRC)
                {
                    ...
                    print_log("MerlinSupport::merlin_wait_uc_active():  CORE 2 CRC verification passed ...\n");
                    polling =0;
                }
            }
            else
        ...
        while (polling)
        {
            ...
            { //uc_dsc_ready_for_cmd@[7]
                ...
                if (calc_crc != MERLIN16_SHORTFIN_UCODE_IMAGE_CRC)
                {
                    ...
                    print_log("MerlinSupport::merlin_wait_uc_active():  CRC verification passed ...\n");
                    polling =0;
                }
            }
            else
...
void merlin_cfg_lane_ram_var (phy_dev_t *phy_dev, uint16_t an_enabled)
{
    ...
    //media type[1:0]: 00-pcb trace; 01-copper; 10-optics
...
nPPI or SFI optical: dfe_on = 0; media_type = "optical", unrealiable_los based on application
    ...
    print_log("MerlinSupport::merlin_cfg_lane_ram_var():  program lane_config_word 0x%x to ram address 0x%x ...\n", lane_config_word, lane_var_ram_base);
...
void merlin_chk_lane_link_status(phy_dev_t *phy_dev)
{
    ...
    for(i=0; i<100; i++) {  /* To filter out certain speed false link up */
        rd_data = host_reg_read(rd_addr);
    ...
    if (phy_dev->link)
    {
        ...
        if (rd_data & MERLIN_MISC_SPEED_10G)
            phy_dev->speed = PHY_SPEED_10000;
        else if (rd_data & MERLIN_MISC_SPEED_5G)
            phy_dev->speed = PHY_SPEED_5000;
        else if (rd_data & MERLIN_MISC_SPEED_2P5G)
            phy_dev->speed = PHY_SPEED_2500;
        else if (rd_data & MERLIN_MISC_SPEED_1G)
            phy_dev->speed = PHY_SPEED_1000;
        else if (rd_data & MERLIN_MISC_SPEED_100M)
            phy_dev->speed = PHY_SPEED_100;
        else if (rd_data & MERLIN_MISC_SPEED_10M)
...
1:  5G baue rate
    ...
    //print_log("MerlinSupport::merlin_chip_init(): Step 7.b wait for uc_active = 1 \n");

    //--Step 7.c Initialize software information table for the micro: merlin16_shortfin_init_merlin16_shortfin_info()
    //print_log("MerlinSupport::merlin_chip_init(): Step 7.c Initialize software information table for the micro \n");
    ...
    if (parse_sim_opts("-d separate_vco")) {
        if (vco_12p5g) {
            print_log("MerlinSupport::merlin_chip_init(): Initialize 12.5G VCO programming \n");
    ...
    //--- Step 1 PLL/PMD setup configuration
    if (parse_sim_opts("-d ML_REFCLK_50")) {
        if (parse_sim_opts("-d separate_vco"))
        {
            if (vco_12p5g)
            {
                print_log("MerlinSupport::merlin_chip_init(): PMD Setup 50MHz, 12.5GHz VCO programming \n");
                merlin_reg_prog(PMD_setup_50_12p5_VCO, CoreNum, 0x0); // If not 10G
            }
            else
            {
                print_log("MerlinSupport::merlin_chip_init(): PMD Setup 50MHz, 10.3125GHz VCO programming \n");
        ...
        if (parse_sim_opts("-d ML_C_VCO_12P5") || vco_12p5g) {
            print_log("MerlinSupport::merlin_chip_init(): PMD Setup 400MHz, 12.5GHz VCO programming \n");
            merlin_reg_prog(PMD_setup_400_12p5_VCO, CoreNum, 0x0);
        } else {
            print_log("MerlinSupport::merlin_chip_init(): PMD Setup 400MHz, 10.3125GHz VCO programming \n");
            merlin_reg_prog(PMD_setup_400_10p3125_VCO, CoreNum, 0x0);
        }
    } else {  //default to 156.25MHz
        if (parse_sim_opts("-d ML_C_VCO_12P5") || vco_12p5g) {
            print_log("MerlinSupport::merlin_chip_init(): PMD Setup 156.25MHz, 12.5GHz VCO programming \n");
            merlin_reg_prog(PMD_setup_156p25_12p5_VCO, CoreNum, 0x0);
        } else {
            print_log("MerlinSupport::merlin_chip_init(): PMD Setup 156.25MHz, 10.3125GHz VCO programming \n");
    ...
    if (parse_sim_opts("-d MERLIN_LOAD_FIRMWARE") ) {
        ...
        print_log("MerlinSupport::merlin_chip_init(): Step 9. Configure Core level regsiter \n");
        ...
        print_log("MerlinSupport::merlin_chip_init(): Step 10. Set core_congif_from_pcs \n");
        if (parse_sim_opts("-d ML_C_VCO_12P5") || vco_12p5g) {
        ...
        print_log("MerlinSupport::merlin_chip_init(): RAM variable vco_rate is %d \n", vco_rate);
    ...
    print_log("MerlinSupport::merlin_chip_init(): Step 12. Reset Datapath (core) \n");
    merlin_reset_datapath_core (CoreNum);

    //--- Step 13. Lane Configuration
    print_log("MerlinSupport::merlin_chip_init(): Step 13. Lane Configuration \n");
    if (parse_sim_opts("-d MERLIN_LOAD_FIRMWARE")) {
        //--- Step 13.a. Configure lane registers
        print_log("MerlinSupport::merlin_chip_init(): Step 13.a. Configure lane registers \n");
        //Lane General
        ...
        #if 0
        print_log("MerlinSupport::merlin_chip_init(): Step 13.b. Configure lane's micro RAM vaiables \n");
        ...
        #endif
        print_log("MerlinSupport::merlin_chip_init(): RAM variable an_enabled is %d \n", phy_dev->an_enabled);
...
int merlin_speed_set(phy_dev_t *phy_dev, phy_speed_t speed, phy_duplex_t duplex)
{
    ...
    return merline_speed_set_core(phy_dev);
}
...
int merlin16_serdes_init(phy_dev_t *phy_dev)
{
    ...
    merlin_core_init(CoreNum, HOST_MDIO_PHY_ADDR(CoreNum), 0x0); //no lane offset
    timeout_ns(1000);
    ...
    /* Set Broadcast MDIO address to 0 to avoid conflicting with our 0x1f system policy */
    merlin_pmi_write16(CoreNum, 0x0, 0x1, 0xffdc, 0, 0xffe0);

    if (parse_sim_opts("-d MERLIN_LOAD_FIRMWARE") ) {
        //--- --- Load firmware ?  TBD
        //--- Step 4. Assert micro reset (merlin16_shortfin_uc_reset(1))
        print_log("MerlinSupport::merlin_chip_init(): Step 4. Assert micro reset \n");
        merlin_uc_reset(CoreNum, 0x1);

        //---Step 6. Micro code load and verify
        print_log("MerlinSupport::merlin_chip_init(): Step 6. Micro code load and verify \n");
        merlin_load_firmware(phy_dev);


        //---Step 7.a De-assert micro reset
        print_log("MerlinSupport::merlin_chip_init(): Step 7.a De-assert micro reset  \n");
    ...
    print_log("INFO MerlinSupport::merlin_chip_init(): END Merlin Initialization procedure\n");
