...
static int ubus_master_decode_wnd_cfg(int master_port_id, int win, phys_addr_t phys_addr, unsigned int size_power_of_2, int port_id, unsigned int cache_bit_en)
{
    ...
    if(master_addr)
    {
#if IS_BCMCHIP(63158) || IS_BCMCHIP(63178) || IS_BCMCHIP(47622) || IS_BCMCHIP(63146) || IS_BCMCHIP(6756)
...
#if IS_BCMCHIP(63158) || IS_BCMCHIP(63178) || IS_BCMCHIP(47622) || IS_BCMCHIP(63146) || IS_BCMCHIP(4912) || IS_BCMCHIP(6756)
    ...
    *offset += sprintf(buff + *offset, "------------------------------------------------------------------------------------\n");
    ...
    while (ub_mst_addr_map_tbl[i].port_id != -1)
    {
        ...
        for (win = 0; win < 4; win++)
        {
            ...
            *offset += sprintf(buff + *offset, "%-9s (%px) | %08x | %08x | %5s |  %3s   | %x:%x:%x:%x\n", 
...
void bcm_ubus_config(void)
{
    ...
    IS_BCMCHIP(4912) || IS_BCMCHIP(6756)
    ubus_remap_to_biu_cfg_wlu_srcpid(MAX_WLU_SRCPID_NUM, 1);
#endif
#if !IS_BCMCHIP(63178) && !IS_BCMCHIP(47622) && !IS_BCMCHIP(63146) && !IS_BCMCHIP(4912) && \
    !IS_BCMCHIP(6756)
