...
extern void chk_rst_btn_event(void);
...
struct image_header
{
    /* Laider Lai refine unsigned long => uint32_t
    for platform's compatibility */
    uint32_t magic;            /* magic */
    uint32_t header_len;       /* Length of header */
    uint8_t reserved[8];
    uint32_t kernel_chksum;    /* Kernel image chksum */
    uint32_t rootfs_chksum;    /* rootfs image chksum */
    uint32_t kernel_len;       /* Length of kernel */
    uint32_t rootfs_len;       /* Length of rootfs */
    uint32_t image_chksum;     /* checksum across length of image */
    uint32_t header_chksum;    /* checksum across length of header */
};
...
static int ui_cmd_flash_router_image(ui_cmdline_t *cmd, int argc, char *argv[])
{
    ...
    /*foxconn Han edited, apply default board id first*/
    memcpy(board_id_test, DEFAULT_BOARD_ID, PROJECT_ID_LEN);
    ...
    if (1) {
        ...
        uint32_t image_len, image_chksum;

        uint32_t header_len = 0;
        uint32_t board_id_len = 0;
        ...
        /* BSP change the NVRAM API, Laider Lai refine it */
        ...
        if (strncmp(board_id, board_id_test, PROJECT_ID_LEN) != 0) {
            printf("Check board id fail: image board id(%s), on-board id(%s)\n", board_id, board_id_test);
    ...
    memcpy(img_tmp, ptr, res);
    if ((res = writeWholeImage(img_tmp, res) ) != 0)
    ...
    if (res == 0)
    {
        ...
        // Flash FW correctly, disable recovery token
        nvramData.ucUnused2[2] = 0x0;

        printf("%s(%d) start to invoke writeNvramData!\n", __FUNCTION__, __LINE__);
...
#error "SEC_S_SIGNATURE must be 4 byte aligned"
    ...
    printf("%s: entering cfe_fs_find_file\n", __FUNCTION__);
    if (
#ifdef USE_LZ4_DECOMPRESSOR 
    ...
    printf("%s: exiting cfe_fs_find_file\n", __FUNCTION__);
...
static int ui_cmd_mem_test(ui_cmdline_t *cmd,int argc,char *argv[])
{
#define DEFAULT_PATTERN 0x5a5a5a5a
#define PATTERN1        0xacacacac
#define PATTERN2        0xcacacaca

    int times = 0;
    uint32_t free_area = 0, free_size = 0;
    uint8_t* MEM8_BASE;
    uint16_t* MEM16_BASE;
    uint32_t* MEM32_BASE;
    uint8_t pattern8;
    ...
    uint32_t pattern32;
    uint8_t* MEM8_TARGET;
    uint32_t* MEM32_TARGET;
    uint32_t i, j, k, size;

    // Check paremeters
    if (argc < 1)
    {
        printf("Usage: memtest <times>\n");
        return -1;
    }
    ...
    if (times <= 0)
    {
        printf("Times should larger than 0\n");
        return -1;
    }
    printf("Start to verify DRAM R/W with %d times...\n", times);

    // Check the free memory part
    cfe_free_mem_info(&free_area, &free_size);
    MEM8_BASE = (uint8_t*) (uintptr_t)free_area;
    MEM16_BASE = (uint16_t*) (uintptr_t)free_area;
    MEM32_BASE = (uint32_t*) (uintptr_t)free_area;
    printf("Only test the free memory part: 0x%08lX - 0x%08lX, size %lu\n", free_area, free_area + free_size - 1, free_size);

    // Start memory test patterns
    size = free_size >> 2;
    ...
    for (k = 0; k < times; k++)
    {
        ...
        // Default pattern
        printf("---%d times default pattern %08lX start\n", k + 1, DEFAULT_PATTERN);
        for (i = 0; i < size; i++)
            MEM32_BASE[i] = DEFAULT_PATTERN;
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != DEFAULT_PATTERN) {
                printf("Error, address:%08lX value:%08lX pattern:%08lX\n", &MEM32_BASE[i], MEM32_BASE[i], DEFAULT_PATTERN);
                return -1;
            }
        }

        // Set to 0x0
        printf("---%d times 0x0 pattern start\n", k + 1);
        for (i = 0; i < size; i++)
            MEM32_BASE[i] = 0x0;
        for (i = 0; i < size; i++)
        {
        ...
        // Set to 0xFFFFFFFF
        printf("---%d times 0xFFFFFFFF pattern start\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0xFFFFFFFF)
            {
                printf("Error, address:%08lX value:%08lX pattern:%08lX\n", &MEM32_BASE[i], MEM32_BASE[i], 0xFFFFFFFF);
                return -1;
            } else
                MEM32_BASE[i] = 0x0;
        }

        // (0x00 ~ 0xff) 1 byte pattern
        printf("---%d times (0x00 - 0xff) 1 byte pattern start\n", k + 1);
        pattern8 = 0x00;
        ...
        for (i = 0; i < free_size; i++)
        {
            ...
            {
                printf("Error, address:%08lX value:%02X pattern:%02X\n", &MEM8_BASE[i], MEM8_BASE[i], pattern8 - 1);
                return -1;
            }
        }

        // (0x00 ~ 0xff) 2 byte pattern
        printf("---%d times (0x00 - 0xff) 2 byte pattern start\n", k + 1);
        pattern8 = 0x00;
        for (i = j = 0; i < free_size; i += 2, j++)
        {
            if (MEM8_BASE[i] == pattern8)
                MEM16_BASE[j] = pattern8;
            if (MEM16_BASE[j] != pattern8)
            {
                printf("Error, address:%08lX value:%04X pattern:%02X\n", &MEM16_BASE[j], MEM16_BASE[j], pattern8);
                return -1;
            }
            pattern8 += 2;
        }
        ...
        // (0x00 ~ 0xffff) pattern
        printf("---%d times (0x00 - 0xffff) pattern start\n", k + 1);
        pattern16 = 0x00;
        for (i = 0; i < (free_size >> 1); i++)
        ...
        for (i = 0; i < (free_size >> 1); i++)
        {
            ...
            {
                printf("Error, address:%08lX value:%04X pattern:%04X\n", &MEM16_BASE[i], MEM16_BASE[i], pattern16 - 1);
                return -1;
            }
        }

        // (0x00 ~ 0xffffffff) pattern
        printf("---%d times (0x00 - 0xffffffff) pattern start\n", k + 1);
        pattern32 = 0x00;
        for (i = 0; i < size; i++)
        ...
        for (i = 0; i < size; i++)
        {
            ...
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], pattern32 - 1);
                return -1;
            }
        }

        // 0x11223344 pattern
        printf("---%d times fill 0x11223344 pattern\n", k + 1);
        for (i = 0; i < size; i++)
            MEM32_BASE[i] = 0x11223344;

        // Check and fill 0xa5a5a5a5 pattern
        printf("---%d times check 0x11223344 pattern and fill 0xa5a5a5a5 pattern\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0x11223344)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], 0x11223344);
                return -1;
            }
            else
        ...
        // Check and fill 0x00 byte pattern at offset 0h
        printf("---%d times check 0xa5a5a5a5 pattern and fill 0x00 byte pattern at offset 0h pattern\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0xa5a5a5a5)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], 0xa5a5a5a5);
                return -1;
            }
            else
                MEM8_BASE[i * 4] = 0x00;
        ...
        // Check and fill 0x00 byte pattern at offset 2h
        printf("---%d times check 0xa5a5a500 pattern and fill 0x00 byte pattern at offset 2h pattern\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0xa5a5a500)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], 0xa5a5a500);
                return -1;
            }
            else
        ...
        // Check and fill 0x00 byte pattern at offset 1h
        printf("---%d times check 0xa500a500 pattern and fill 0x00 byte pattern at offset 1h pattern\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0xa500a500)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], 0xa500a500);
                return -1;
            }
            else
        ...
        // Check and fill 0x00 byte pattern at offset 3h
        printf("---%d times check 0xa5000000 pattern and fill 0x00 byte pattern at offset 3h pattern\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0xa5000000)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], 0xa5000000);
                return -1;
            }
            else
        ...
        // Check and fill 0xffff byte pattern at offset 1h
        printf("---%d times check 0x00000000 pattern and fill 0xffff byte pattern at offset 1h pattern\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0x00000000)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], 0x00000000);
                return -1;
            }
            else
        ...
        // Check and fill 0xffff byte pattern at offset 0h
        printf("---%d times check 0xffff0000 pattern and fill 0xffff byte pattern at offset 0h pattern\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0xffff0000)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], 0xffff0000);
                return -1;
            }
            else
        ...
        // Check pattern value
        printf("---%d times check 0xffffffff pattern\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0xffffffff)
            {
        ...
        // Stage 1 ~ 6
        printf("---%d times stage 1, fill pattern1\n", k + 1);
        for (i = 0; i < size; i++)
            MEM32_BASE[i] = PATTERN1;
        printf("---%d times stage 2, check pattern1 and fill pattern2\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != PATTERN1)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], PATTERN1);
                return -1;
            }
            MEM32_BASE[i] = PATTERN2;
        }
        printf("---%d times stage 3, check pattern2 and fill pattern1\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != PATTERN2)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], PATTERN2);
                return -1;
            }
            MEM32_BASE[i] = PATTERN1;
        }
        printf("---%d times stage 4, check pattern1 and fill pattern2\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != PATTERN1)
            {
        ...
        printf("---%d times stage 5, check pattern2 and fill pattern1\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != PATTERN2)
            {
        ...
        printf("---%d times stage 6, check pattern1\n", k + 1);
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != PATTERN1)
            {
        ...
        // 1 & 4 byte combination pattern
        printf("---%d times 1 & 4 byte combination pattern\n", k + 1);
        i = free_area;
        j = 0;

        while (i < (free_area + free_size))
        {
            switch (j)
            {
                case 0:
                    MEM32_TARGET = (uint32_t*) (uintptr_t)i;
                    *MEM32_TARGET = 0x1234abcd;
                    i += 4;
                    break;
                case 1:
                    MEM8_TARGET = (uint8_t*) (uintptr_t)i;
                    *MEM8_TARGET = 0xcd;
                    i += 1;
                    MEM8_TARGET = (uint8_t*) (uintptr_t)i;
                    *MEM8_TARGET = 0xab;
                    ...
                    *MEM8_TARGET = 0x12;
                    i += 1;
                    break;
                // *((uint16_t*) (uintptr_t)i) = 0x0000 have some toolchain issue
                // not support at this platform
                default:
                    printf("Error, should not executed at here\n");
                    break;
            }
            if (j) j = 0; else j = 1;
        }
        ...
        for (i = 0; i < size; i++)
        {
            if (MEM32_BASE[i] != 0x1234abcd)
            {
                printf("Error, address:%08lX value:%08X pattern:%08X\n", &MEM32_BASE[i], MEM32_BASE[i], 0x1234abcd);
                return -1;
            }
        }
...
extern void mr80_ethsw_wan_loopback(void);
//void bcm63xx_run_ex(int breakIntoCfe, int skip_check_memcfg)
int bcm63xx_run_ex(int breakIntoCfe, int skip_check_memcfg) /* Foxconn modified */
{
    int autoRun = 0; /* Foxconn added */
...
/* Exception case handling to detect A0 image on B0 and vice versa */
    ...
    /* For MR80 only
     * Set MR80 WAN port loopback to avoid tx/rx invlaid packets
     */
    mr80_ethsw_wan_loopback();
    ...
    enet_init();
    g_in_cfe = 1;
    autoRun = runDelay(bootInfo.bootDelay);
    /* Foxconn added end */

    if (!breakIntoCfe && autoRun) { /* Foxconn modified */
        run_info_t ar = { 0 };
...
#if 0
        /*foxconn Han removed, 07/09/2019 675X don't need to load test image automatically*/
        /* Foxconn added, Laider Lai */
        // Detect to run test mode or not
        if (!(NVRAM.ucUnused3[1] == 'D' && NVRAM.ucUnused3[0] == 'I')) {
            printf("Test mode is enabled, run tftp to load test image\n");
            ui_docommand("r n 192.168.1.2 vmlinux_rd_boot ramdisk 947622.dtb 0x8000000");
        }
#endif
        // Detect recovery token
        if (NVRAM.ucUnused2[2] == 'W')
            return 1; // boot to tftp recovery mode
        /* Foxconn added end, Laider Lai */

        // RST button event will trigger load default
        chk_rst_btn_event();
