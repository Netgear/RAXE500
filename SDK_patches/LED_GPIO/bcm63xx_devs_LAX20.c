...
void diagnostic_led_toggle(void); /* Foxconn modified */
/*  *********************************************************************
...
/*foxconn Han edited start 07/16/2018 turn on power LED*/
    //printf("\n%s(%d) lights power led\n",__func__,__LINE__);
    printf("** board_device_init power_led_off **\n");
    power_led_off();
    diagnostic_led_on();
/*foxconn Han edited end 07/16/2018*/
...
#define Reset_GPIO 9	
	...
	{	
    	...
    	for( i=0; i<20; i++ )
    	{
            diagnostic_led_toggle();
            cfe_usleep(250000); //delay 250ms
        }
        diagnostic_led_on();
	}
	return;
...
// Foxconn add start, Min-Chang, patch for Quectel LTE module, 2020-01-02
static void fx_init_LTE_module(void)
{
#define GPIO_DIR_031_000	0xFF800500
#define GPIO_DATA_031_000	0xFF800520
#define BIT_27				0x08000000
#define BIT_28				0x10000000
#define BIT_30				0x40000000
#define BIT_31				0x80000000

	volatile uint32_t *gpio_dir = (void *)(GPIO_DIR_031_000);
	volatile uint32_t *gpio_data = (void *)(GPIO_DATA_031_000);
	uint32_t val32;

	printf("%s enter\n", __FUNCTION__);

	/* set GPIO_27, GPIO_28, GPIO_30 and GPIO_31 to output */
	val32 = *gpio_dir;
	val32 |= (BIT_27 | BIT_28 | BIT_30 | BIT_31);
	*gpio_dir = val32;

	/* set GPIO_27=0, GPIO_28=0, GPIO_30=1 and GPIO_31=1 */
	val32 = *gpio_data;
	val32 &= ~(BIT_27 | BIT_28);
	val32 |= (BIT_30 | BIT_31);
	*gpio_data = val32;
...
// Foxconn add end, Min-Chang, patch for Quectel LTE module, 2020-01-02
...
void board_final_init(int force_cfe)
{
    ...
    setAllLedsOff();
    //setPowerOnLedOn();
    printf("** board_final_init power_led_off **\n");
    power_led_off();
    diagnostic_led_on();

	fx_init_LTE_module(); // Add by Min-Chang, patch for Quectel LTE module, 2020-01-02
...
void power_led_off(void)
{
    bcm_gpio_set_dir(GPIO_PWR_LED, 1);
    bcm_gpio_set_data(GPIO_PWR_LED, 0);
    cfe_usleep(100000); //delay 100ms
}
...
{
    bcm_gpio_set_dir(GPIO_PWR_LED, 1);
    bcm_gpio_set_data(GPIO_PWR_LED, state);
}

#define AL_AH_REG		0xFF803014
#define SW_INPUT_REG	0xFF803010

#define LED_R_PWR_REG		0xff803190		/* GPIO 23 */
#define LED_G_PWR_REG		0xff803180		/* GPIO 22 */
#define LED_B_PWR_REG		0xff8030c0		/* GPIO 10 */

#define LED_R_WAN_REG		0xff803110		/* GPIO 15 */
#define LED_G_WAN_REG		0xff803050		/* GPIO 3 */
#define LED_B_WAN_REG		0xff8031a0		/* GPIO 24 */

#define LED_Bit_MASK	0x01C08408
#define LED_N_Bit_MASK	0xFE3F7BF7
...
void diagnostic_led_on(void)
{
	...
	volatile uint32_t *cled_led_r1_reg = (void *)(LED_R_PWR_REG);
	volatile uint32_t *cled_led_g1_reg = (void *)(LED_G_PWR_REG);
	volatile uint32_t *cled_led_b1_reg = (void *)(LED_B_PWR_REG);
	volatile uint32_t *cled_act_cfg = (void *)(LED_ACT_CFG);
	uint32_t val32;

	/* set LED22(R), LED23(G), LED10(B) as active low */
	...
	*cled_input_reg = val32;
	...
	/* Config Power LED amber */
	*cled_led_r1_reg = (LED_R_BRIGHTNESS << 6);
	*cled_led_g1_reg = (LED_G_BRIGHTNESS << 6);
	*cled_led_b1_reg = (LED_B_BRIGHTNESS << 6);//((0x80 - LED_B_BRIGHTNESS) << 6);

        /* clear WAN LED reg */
        cled_led_r1_reg = (void *)(LED_R_WAN_REG);
	cled_led_g1_reg = (void *)(LED_G_WAN_REG);
	cled_led_b1_reg = (void *)(LED_B_WAN_REG);
...
void diagnostic_led_off(void)
{
	volatile uint32_t *cled_al_ah_reg = (void *)(AL_AH_REG);
	volatile uint32_t *cled_input_reg = (void *)(SW_INPUT_REG);
	volatile uint32_t *cled_led_r1_reg = (void *)(LED_R_WAN_REG);
	volatile uint32_t *cled_led_g1_reg = (void *)(LED_G_WAN_REG);
	volatile uint32_t *cled_led_b1_reg = (void *)(LED_B_WAN_REG);
	volatile uint32_t *cled_act_cfg = (void *)(LED_ACT_CFG);
	uint32_t val32;
	...
	*cled_led_r1_reg = 0;
	*cled_led_g1_reg = 0;
	*cled_led_b1_reg = 0;//((0x80 - LED_B_BRIGHTNESS) << 6);
