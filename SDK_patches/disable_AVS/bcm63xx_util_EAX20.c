...
void cfe_free_mem_info(uint32_t* free_area, uint32_t* free_size)
{
    *free_area = MEMORY_AREA_FREE_MEM;
    *free_size = (MEMORY_AREA_RSRV_ADDR + cfe_get_sdram_size()) - MEMORY_AREA_FREE_MEM;
}
/* Foxconn */
...
// which has the pre-Cferom binary
    ...
    if(( flash_get_flash_type() == FLASH_IFC_NAND ) ||
	   ...
	   ((unsigned char )NVRAM.szBoardId[1]) == 0xff) {
		...
		if( checkImageSizeForNand(NP_ROOTFS_1, imagePtr, imageSize) != 0 )
			goto err_out;
#else
