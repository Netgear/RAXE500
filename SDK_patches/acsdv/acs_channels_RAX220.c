...
#define ACS_CHANNEL_12 12
#define ACS_CHANNEL_13 13
#define ACS_CHANNEL_14 14
...
acs_invalidate_candidates(acs_chaninfo_t *c_info, ch_candidate_t *candi, int bw)
{
	...
	for (i = 0; i < c_info->c_count[bw]; i++) {
		...
		if (BAND_2G(rsi->band_type)) {
			...
			if (nvram_match("acs_fxcn_hack", "1") &&
					((chan.control == ACS_CHANNEL_1) ||
					(chan.control == ACS_CHANNEL_6) ||
					(chan.control == ACS_CHANNEL_11) ||
					(chan.control == ACS_CHANNEL_12) ||
					(chan.control == ACS_CHANNEL_13) ||
					(chan.control == ACS_CHANNEL_14))) 
			{

            			ACSD_INFO("banning chanspec %x because of acs_fxcn_hack \n",candi[i].chspec);
				candi[i].valid = FALSE;
				candi[i].reason |= ACS_INVALID_EXCL;
        	}	
