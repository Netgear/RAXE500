...
#define DEFAULT_CRASH_LOG_DIR  "/data"
...
static void startAirIQ(void) {
    ...
    if(detectApp("airiq_service") && (nv_value = nvram_unf_get("airiq_service_enable")))
    {
        if(!strcmp(nv_value,"1") || !strcmp(nv_value,"y"))
        {
            ...
            {
                if (DPD_WL_RADIO_DISABLED(index))
                    continue;
...
#ifdef CONFIG_HOSTAPD
    {
        ...
        if (hapd != NULL)
        {
            if (strcmp(hapd, "0") != 0)
            {
                ...
                if (nvram_match("acos_wps_use_acos_wpsd", "1"))
                    bcmSystem("acos_wpsd &");
                else
                    bcmSystem("wps_pbcd");
#endif
...
#ifdef BUILD_RDKWIFI
    nvram_set("ecbd_enable","1");
    bcmSystem("ecbd&");
#endif

#ifdef BUILD_WLDATAELD
    if(detectApp("wldataeld"))
        if(!sysUtil_isFilePresent("/var/wldataeld_notify"))
            bcmSystem("wldataeld -n &");
#endif
...
void wlssk_stop_services(unsigned int services)
{
    stop_debug_monitor();
#ifdef BUILD_RDKWIFI
    bcmSystem("killall ecbd 2>/dev/null");
#endif
...
#ifdef CONFIG_HOSTAPD
	if (nvram_match("acos_wps_use_acos_wpsd", "1"))
		bcmSystem("killall acos_wpsd");
	else
		bcmSystem("killall wps_pbcd");
    stop_hapd_wpasupp();
#endif  /* CONFIG_HOSTAPD */
    bcmSystem("killall -q -9 nas 2>/dev/null");
    bcmSystem("killall -q acsd 2>/dev/null");
    bcmSystem("killall -q acsd2 2>/dev/null");
    bcmSystem("killall -q escand 2>/dev/null");
