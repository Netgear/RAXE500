...
{
    int rc;
    rc = bdmf_codegen_pygen_fprintf(p, BDMF_TAB "def %s_add(self, value, index=''):\n", a->name);
    /* Validate value */
    rc = rc ? rc : bdmf_codegen_pygen_add_attr_validators(p, a, NULL, "value", BDMF_TAB2);
    rc = rc ? rc : bdmf_codegen_pygen_fprintf(p, BDMF_TAB2 "super(%s, self).attr_add('%s', value, index)\n",
