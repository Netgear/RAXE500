...
* extern prototypes
*/
extern int bdmf_history_status_get(void);
extern void bdmf_attr_val_to_s_full_dump_set(int val_to_s_full_dump);
...
void __bdmf_mod_object_ref_list_dump(struct bdmf_object *obj)
{
    struct bdmf_ref *ref, *ref_tmp;

    bdmf_trace("Object %s is referenced by the following clients:\n", obj->name);
    bdmf_trace("=============================================================\n");
    DLIST_FOREACH_SAFE(ref, &obj->clients, client_list, ref_tmp)
    {
        if (bdmf_global_trace_level == bdmf_trace_level_debug)
        {
            bdmf_trace("REF <%p>, client %s <%p>, ref %s <%p>\n", ref, ref->client->name, ref->client,
                ref->ref_obj->name, ref->ref_obj); 
        }
        else
        {
            bdmf_trace("%s\n", ref->client->name);
    ...
    bdmf_trace("Following objects are referenced by the object %s:\n", obj->name);
    bdmf_trace("=============================================================\n");
    DLIST_FOREACH_SAFE(ref, &obj->references, ref_list, ref_tmp)
    {
        if (bdmf_global_trace_level == bdmf_trace_level_debug)
        {
            bdmf_trace("REF <%p>, client %s <%p>, ref %s <%p>\n", ref, ref->client->name, ref->client,
...
static int bdmf_mon_object_ref_list(bdmf_session_handle session,
                                    const bdmfmon_cmd_parm_t parm[],  uint16_t n_parms)
{
    char *type_discr=parm[0].value.string;
    struct bdmf_object *obj = NULL;
    ...
    if (rc || !obj)
        return rc;
    __bdmf_mod_object_ref_list_dump(obj);
    bdmf_put(obj);
    return 0;
}
                                    ...
                                    const bdmfmon_cmd_parm_t parm[],  uint16_t n_parms)
{
    ...
    {
        /* non-zero statistics are represented as BDMF_ATTR_CONFIG | BDMF_ATTR_CLASS_STAT */
        if (cl == (BDMF_ATTR_CONFIG | BDMF_ATTR_CLASS_STAT))
        {
            cl = BDMF_ATTR_CLASS_STAT;
            nzstat = 1;
        }
        class_level |= cl;
    }
    ...
    const char *drv_name)
{
    ...
    while((obj=bdmf_get_next(drv, obj, NULL)))
    {
        ...
        _bdmf_mon_object_examine(session, obj, NULL, NULL,
            BDMF_ATTR_CLASS_STAT, 1, 0, BDMFMON_EXAMINE_AG_FORMAT_LINE, 10, 1, &print_obj_name);
    }
    return rc;
}
...
/* Enable/Disabpe val_to_str full extension  object type help
BDMFMON_MAKE_PARM("enable", "full dump value : yes enable , no disable")
*/
static int bdmf_mon_val_to_str_full_enable(bdmf_session_handle session,
                               const bdmfmon_cmd_parm_t parm[],  uint16_t n_parms)
{
    int val_to_s_full_dump = (int)parm[0].value.number;

#if defined(BDMF_HISTORY)
    if (bdmf_history_status_get())
    {
        bdmf_session_print(session, "Operation not supported in history mode\n");
        return BDMF_ERR_NOT_SUPPORTED;
    }
#endif

    bdmf_attr_val_to_s_full_dump_set(val_to_s_full_dump);
...
bdmfmon_handle_t bdmf_flow_mon_init(bdmfmon_handle_t root_dir)
{
    ...
    }
    {
        static bdmfmon_enum_val_t attr_class_table[] = {
            ...
            { .name="stat",   .val=BDMF_ATTR_CLASS_STAT},           /* Statistic */
            { .name="nzstat", .val=BDMF_ATTR_CLASS_STAT | BDMF_ATTR_CONFIG}, /* Non-zero statistic */
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        ...
        bdmfmon_cmd_add(bdmf_dir, "Refs", bdmf_mon_object_ref_list,
    ...
    {
        static bdmfmon_cmd_parm_t parms[]={
            BDMFMON_MAKE_PARM_ENUM_DEFVAL("enable", "val2str full dump  : yes enable , no disable", bdmfmon_enum_bool_table,
                BDMFMON_PARM_FLAG_OPTIONAL, "no"),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        bdmfmon_cmd_add(bdmf_dir, "val2s_full", bdmf_mon_val_to_str_full_enable,
            "Enable dissable val to str full extensions prints",
