...
const static char *bdmf_attr_format_p = "%px";
const static char *bdmf_attr_format_invalid = "*Invalid use of ->format\n";

static int is_val_to_s_full_dump = 0; /* when it's 1 it disables short version for same char multiple prints like 0[8] 8 zeros */
...
/* is_full_dump getter and setter */
int bdmf_attr_val_to_s_full_dump_get(void)
{
    return is_val_to_s_full_dump;
}

void bdmf_attr_val_to_s_full_dump_set(int full_dump_val)
{
    is_val_to_s_full_dump = full_dump_val;
}
    ...
    uint32_t size)
{
    ...
    if (is_val_to_s_full_dump)
    {
        for (i = 0; i < ad->size; i++)
            sbuf += sprintf(sbuf, "%2.2x", hbuf[i]);
    }
    else
    {
        for (i = 0; i < ad->size; i++)
...
/***********************************************************************************/
/* Set attribute default params functions                                          */
/***********************************************************************************/
/***********************************************************************************/
/* _bdmf_attr_set_def_size(attr, drv) - set attribute size based on attribute type */
/***********************************************************************************/
static inline int _bdmf_attr_set_def_size(struct bdmf_attr *attr, struct bdmf_type *drv)
{
    switch (attr->type)
    {
...
/************************************************************************************************/
/* _bdmf_attr_set_def_index_size(attr, size) - set attribute index_size based on attribute type */
/************************************************************************************************/
static void inline _bdmf_attr_set_def_index_size(struct bdmf_attr *attr)
{
...
/************************************************************************************************/
/* _bdmf_attr_set_ts_format_size(attr, size) - set attribute ts_format based on attribute type  */
/************************************************************************************************/
static int inline _bdmf_attr_set_ts_format_size(struct bdmf_attr *attr, struct bdmf_type *drv, uint32_t flags, int is_index)
{
    if (is_index == 0)
    {
        switch (attr->type) 
        ...
        case bdmf_attr_number:
            attr->ts.format = attr_dft_n_format_by_size(attr->size, flags);
            break;
...
/*****************************************************************************************************/
/* _bdmf_attr_set_s_to_val(attr, is_index) - set attribute s_to_val callback based on attribute type */
/*****************************************************************************************************/
static F_s_to_val_t default_func_s_to_val[bdmf_attr_last_type][2] = { [bdmf_attr_number] = {_bdmf_attr_s_to_val_n, _bdmf_attr_s_to_index_n},
                               [bdmf_attr_string] = {_bdmf_attr_s_to_val_s, _bdmf_attr_s_to_val_s},
                               [bdmf_attr_buffer] = {_bdmf_attr_s_to_val_buffer, _bdmf_attr_s_to_val_buffer},
                               [bdmf_attr_pointer] = {_bdmf_attr_s_to_val_n, _bdmf_attr_s_to_index_n},
                               [bdmf_attr_object] = {bdmf_attr_s_to_val_object, bdmf_attr_s_to_val_object},
                               [bdmf_attr_ether_addr] = {_bdmf_attr_s_to_val_mac, _bdmf_attr_s_to_val_mac},
                               [bdmf_attr_ip_addr] = {_bdmf_attr_s_to_val_ip, _bdmf_attr_s_to_val_ip},
                               [bdmf_attr_ipv4_addr] = {_bdmf_attr_s_to_val_ipv4, _bdmf_attr_s_to_val_ipv4},
                               [bdmf_attr_ipv6_addr] = {_bdmf_attr_s_to_val_ipv6, _bdmf_attr_s_to_val_ipv6},
                               [bdmf_attr_boolean] = {NULL, NULL },
                               [bdmf_attr_enum] = {_bdmf_attr_s_to_val_enum, _bdmf_attr_s_to_index_enum},
                               [bdmf_attr_dyn_enum] = {_bdmf_attr_s_to_val_dyn_enum, NULL},
                               [bdmf_attr_enum_mask] = {_bdmf_attr_s_to_val_enum_mask, _bdmf_attr_s_to_index_enum_mask},
                               [bdmf_attr_aggregate] = {_bdmf_attr_s_to_val_aggregate, _bdmf_attr_s_to_index_aggregate},
                               [bdmf_attr_custom] = {NULL, NULL} };

static inline void _bdmf_attr_set_s_to_val(struct bdmf_attr *attr, F_s_to_val_t *f_s_to_val, int is_index)
{
    bdmf_attr_type_t attr_type = is_index ? attr->index_type : attr->type;

    *f_s_to_val = default_func_s_to_val[attr_type][is_index & 1];
}

/*************************************************************************************************/
/* _bdmf_attr_validate_size(attr) validate if attr size configured ok                            */
/*************************************************************************************************/
static int _bdmf_attr_validate_size(struct bdmf_attr *attr)
{
    int rc = BDMF_ERR_OK;
...
/**************************************************************************************************/
/* _bdmf_attr_set_val_to_s(attr, size) - set attribute val_to_s callback based on attribute type  */
/**************************************************************************************************/
static  F_val_to_s_t default_func_val_to_s[bdmf_attr_last_type][2] = { [bdmf_attr_number] = {_bdmf_attr_val_to_s_n, _bdmf_attr_index_to_s_n},
                               [bdmf_attr_string] = {_bdmf_attr_val_to_s_s, _bdmf_attr_val_to_s_s},
                               [bdmf_attr_buffer] = {_bdmf_attr_val_to_s_buffer, _bdmf_attr_val_to_s_buffer},
                               [bdmf_attr_pointer] = {_bdmf_attr_val_to_s_n, _bdmf_attr_index_to_s_n},
                               [bdmf_attr_object] = {bdmf_attr_val_to_s_object, bdmf_attr_val_to_s_object},
                               [bdmf_attr_ether_addr] = {_bdmf_attr_val_to_s_mac, _bdmf_attr_val_to_s_mac},
                               [bdmf_attr_ip_addr] = {_bdmf_attr_val_to_s_ip, _bdmf_attr_val_to_s_ip},
                               [bdmf_attr_ipv4_addr] = {_bdmf_attr_val_to_s_ipv4, _bdmf_attr_val_to_s_ipv4},
                               [bdmf_attr_ipv6_addr] = {_bdmf_attr_val_to_s_ipv6, _bdmf_attr_val_to_s_ipv6},
                               [bdmf_attr_boolean] = {NULL, NULL},
                               [bdmf_attr_enum] = {_bdmf_attr_val_to_s_enum, _bdmf_attr_index_to_s_enum},
                               [bdmf_attr_dyn_enum] = {_bdmf_attr_val_to_s_dyn_enum, NULL},
                               [bdmf_attr_enum_mask] = {_bdmf_attr_val_to_s_enum_mask, _bdmf_attr_index_to_s_enum_mask},
                               [bdmf_attr_aggregate] = {_bdmf_attr_val_to_s_aggregate, _bdmf_attr_index_to_s_aggregate},
                               [bdmf_attr_custom] = {NULL, NULL} };

static inline void _bdmf_attr_set_val_to_s(struct bdmf_attr *attr, F_val_to_s_t *f_val_to_s, int is_index)
{
    bdmf_attr_type_t attr_type = is_index ? attr->index_type : attr->type;

    *f_val_to_s = default_func_val_to_s[attr_type][is_index & 1]; /* possible values for is_index 0 1 */
}

int bdmf_attr_make(struct bdmf_type *drv, struct bdmf_attr *attr)
{
    int rc;
    int set_user_hist_val_to_s = 0;
    int set_user_hist_index_to_s = 0;
    ...
    if (!attr->size)
    {
        rc = _bdmf_attr_set_def_size(attr, drv);
        if (rc)
           return rc;
    }
    ...
    if (!attr->index_size)
        _bdmf_attr_set_def_index_size(attr);

    if (attr->index_size < sizeof(bdmf_index))
        attr->index_size = sizeof(bdmf_index);

    /* Check size */
    rc = _bdmf_attr_validate_size(attr);
    if (rc)
    ...
    if (!attr->ts.format)
    {
        rc = _bdmf_attr_set_ts_format_size(attr, drv, attr->flags, 0);
        if (rc)
            return rc;
    }
    ...
    {
       /* Set defaults format for index conversion */
        rc = _bdmf_attr_set_ts_format_size(attr, drv, 0, 1);
        if (rc)
            return rc;
    }
    ...
    if (!attr->s_to_val)
        _bdmf_attr_set_s_to_val(attr, &attr->s_to_val, 0);
    else
        set_user_hist_val_to_s = 1;

    _bdmf_attr_set_val_to_s(attr, &attr->hist_val_to_s, 0);
    if (!attr->val_to_s)
        attr->val_to_s = attr->hist_val_to_s;
    else if (set_user_hist_val_to_s)
        attr->hist_val_to_s = attr->val_to_s; /* we assume that both val_to_s and s_to_val callbacks are overriden */
                                              /* and should be paired */

    if (!attr->s_to_index)
        _bdmf_attr_set_s_to_val(attr, &attr->s_to_index, 1); /* set attribute hist_s_to_index callback */
    else
        set_user_hist_index_to_s = 1;

    _bdmf_attr_set_val_to_s(attr, &attr->hist_index_to_s, 1);
    if (!attr->index_to_s)
        attr->index_to_s = attr->hist_index_to_s;
    else if (set_user_hist_index_to_s)
        attr->hist_index_to_s = attr->index_to_s; /* we assume that both index_to_s and s_to_index callbacks are overriden */
                                                  /* and should be paired */
    if (!attr->read)
    {
