...
static void cpu_ring_print_phys_addr(void *shell_priv, uint32_t addr_hi, uint32_t addr_low)
{
    ...
    shell_print(shell_priv, "\tpacket DDR phys: 0x%px\n", (uintptr_t *)phys_addr);
    shell_print(shell_priv, "\tpacket DDR virt uncached address: 0x%px\n", (void *)PHYS_TO_UNCACHED(phys_addr));
}
...
#if defined(XRDP) && !defined(_CFE_)
    if ((ring_type == rdpa_ring_recycle) || (ring_type == rdpa_ring_feed))
    {
        /* make base aligned to 64 byte */
        pDescriptor->base =  rdp_mm_aligned_alloc((((pDescriptor->size_of_entry * entries) + RW_INDEX_SIZE*2 + 63) >> 6) << 6, &phy_addr);
...
#endif
#ifndef CONFIG_BRCM_QEMU /* JIRA SWBCACPE-36303*/
...
void rdp_recycle_buf_to_feed(void *pdata)
{
    ...
    {
        rdp_cpu_get_read_idx(FEED_RING_ID, rdpa_ring_feed, &feed_ring_descr->shadow_read_idx);
        read_idx  = feed_ring_descr->shadow_read_idx;
        write_idx = feed_ring_descr->shadow_write_idx;
        num_entries = feed_ring_descr->num_of_entries;
        if (is_rdp_cpu_feed_ring_occupancy_below_threshold(read_idx,write_idx,num_entries)) 
        {
            i = assign_packets_to_feed_ring(16);
            threshold_recycle += i;
        }
    }
#endif
